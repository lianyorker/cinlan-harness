...
function schemaAlias(world: World, ctx: FileCtx, name: string): { ctx: FileCtx; expr: ts.Expression } {
  ...
  if (imported.specifier.startsWith('.')) {
    const sourceRoot = resolve(world.scanRoot, ctx.rel.split('/').slice(0, 3).join('/'), 'src')
    const sourcePath = relative(sourceRoot, resolve(dirname(ctx.abs), imported.specifier))
    if (!imported.specifier.endsWith('.ts') || imported.specifier.endsWith('.d.ts')
      || isAbsolute(sourcePath) || sourcePath === '..' || sourcePath.startsWith(`..${sep}`)) {
      throw new Error(`schema import '${imported.specifier}' must be an explicit .ts source inside the importing package's src directory`)
    }
    target = loadRelative(world, ctx, imported.specifier)
  } else {
    target = loadWorkspaceSource(world, ctx, imported.specifier)
  }
 ...
 * from package-local files or public workspace source paths, and `z.intersect([X.Config, …])`; errors on
