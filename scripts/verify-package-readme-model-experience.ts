...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/api/device-capabilities-controller': { kind: 'none', reason: 'Read-only device readiness Remote; registers no prompt, model tool, or Session event.' },
  ...
  'packages/api/usage-controller': { kind: 'none', reason: 'Authenticated read-only reports expose recorded usage without modifying model requests.' },
  'packages/session-query/usage-query': { kind: 'none', reason: 'Read-only aggregation derives reports from existing Session logs without activating Agents.' },
  'packages/client/ui-settings-usage': { kind: 'none', reason: 'Browser reports and CSV exports do not change Session logs or model requests.' },
  'packages/client/ui-settings-terminal': { kind: 'indirect', reason: 'Saved shell preferences affect subsequent UI and agent-owned terminal creation without adding prompts or tools.' },
  'packages/client/keyboard': { kind: 'none', reason: 'Browser command matching dispatches existing UI actions and registers no model-facing context.' },
  'packages/api/voice-controller': { kind: 'none', reason: 'Voice management and transcription return browser data; submitting a draft remains a separate user action.' },
  'packages/api/integration-preflight-controller': { kind: 'none', reason: 'Human readiness probes report installed integrations without invoking models or adding Session context.' },
  'packages/mcp/mcp-management': { kind: 'indirect', reason: 'The manager delegates discovered tool schemas and execution to the MCP client bridge.' },
  'packages/api/mcp-controller': { kind: 'indirect', reason: 'Browser Remote operations delegate connection changes and model effects to the MCP manager.' },
  'packages/client/ui-settings-mcp': { kind: 'indirect', reason: 'Browser controls delegate changes to available tools to the MCP manager.' },
  'packages/bundle/execution-host-app': { kind: 'none', reason: 'The standalone worker exposes only bounded human directory inspection and mounts no Agent.' },
  'packages/api/execution-host-controller': { kind: 'none', reason: 'Human inspection and connection commands add no model prompt, tool, or Session event.' },
  'packages/api/sidebar-terminal-controller': { kind: 'none', reason: 'The browser terminal carrier transports human PTY I/O without changing model tool requests.' },
  'packages/api/sidebar-git-controller': { kind: 'none', reason: 'Explicit browser Git operations register no model tool or model context.' },
  'packages/git/sidebar-git': { kind: 'none', reason: 'Human Git mutations and observations are separate from the read-only model tool capability.' },
  'packages/terminal/sidebar-terminals': { kind: 'none', reason: 'The human terminal contract forwards PTY I/O and does not compose model requests.' },
  'packages/execution-host/execution-host-targets': { kind: 'none', reason: 'Saved SSH targets expose bounded human inspection and provide no remote Agent routing.' },
  'packages/execution-host/execution-host-worker': { kind: 'none', reason: 'The standalone worker mounts no Agent and handles only bounded inspection frames.' },
  'packages/client/ui-settings-hosts': { kind: 'none', reason: 'Host management controls perform human connection and inspection operations only.' },
  'packages/client/ui-floating-workspace': { kind: 'none', reason: 'A window displays the existing app and Session without adding model context.' },
  'packages/client/ui-keybindings': { kind: 'none', reason: 'Shortcut preferences dispatch existing UI commands without registering model inputs.' },
  'packages/client/ui-notifications': { kind: 'none', reason: 'Notification controls and browser delivery register no model inputs.' },
  'packages/notifications/notifications': { kind: 'none', reason: 'The sole notification schema owner registers browser preference data only.' },
  'packages/client/ui-orchestration': { kind: 'indirect', reason: 'Saved preferences are consumed by the existing preset and orchestration owners.' },
  'packages/client/ui-integrations': { kind: 'none', reason: 'Human readiness checks only report integration status and do not change settings or model inputs.' },
  'packages/client/ui-git-settings': { kind: 'indirect', reason: 'Saved Git preferences reach Worktree Task creation and explicit sidebar Git operations.' },
  'packages/git/git-settings': { kind: 'indirect', reason: 'The schema owner supplies preferences to Worktree Task and sidebar Git consumers.' },
  'packages/api/automation-controller': { kind: 'indirect', reason: 'Remote commands delegate Agent invocation and logged user input to the automation runtime.' },
  'packages/client/ui-settings-automation': { kind: 'indirect', reason: 'Creation and explicit run controls delegate Session creation to the automation runtime.' },
  ...
  'packages/work-items/work-items': { kind: 'indirect', reason: 'The registry supplies normalized records and write receipts to dsh-tool-work-items.' },
  'packages/work-items/work-items-github': { kind: 'indirect', reason: 'The GitHub provider returns records through the Work Items registry; the tool Consumer owns rendering.' },
  'packages/work-items/work-items-gitlab': { kind: 'indirect', reason: 'The GitLab provider returns records through the Work Items registry; the tool Consumer owns rendering.' },
  'packages/work-items/work-items-linear': { kind: 'indirect', reason: 'The Linear provider returns records through the Work Items registry; the tool Consumer owns rendering.' },
  'packages/bundle/cinlan-work-items': { kind: 'indirect', reason: 'The bundle composes Work Items tools; the tool Consumer owns model input.' },
  'packages/api/worktree-task-controller': { kind: 'none', reason: 'The Worktree Task Remote controller registers no prompt, tool, or Session event.' },
  'packages/api/workspace-isolation-controller': { kind: 'none', reason: 'The Workspace Isolation Remote controller registers no prompt, tool, or Session event.' },
  'packages/artifact/artifact': { kind: 'none', reason: 'The artifact metadata service registers no prompt, tool, or Session event.' },
  'packages/artifact/artifact-memory': { kind: 'none', reason: 'The in-memory artifact provider stores evidence for callers and registers no model-facing input.' },
  'packages/bundle/security-findings': { kind: 'indirect', reason: 'The bundle carries finding providers and tools; those inserted packages own model-facing behavior.' },
  'packages/bundle/security-research': { kind: 'indirect', reason: 'The profile layer carries security providers and consumers; each inserted package owns its model-facing behavior.' },
  'packages/bundle/security-skills': { kind: 'indirect', reason: 'The bundle carries the skill provider; the skill consumer owns model-facing rendering.' },
  'packages/bundle/security-workflow': { kind: 'indirect', reason: 'The bundle carries the workflow prompt plugin, which owns its model-facing prompt.' },
  'packages/bundle/vuln-kb': { kind: 'indirect', reason: 'The bundle carries vulnerability knowledge-base providers and tools; those packages own model-facing behavior.' },
  'packages/client/ui-right-sidebar': { kind: 'none', reason: 'The browser sidebar contributors register no prompt, tool, or Session event.' },
  'packages/client/ui-settings-security': { kind: 'none', reason: 'The browser security settings page registers no prompt, tool, or Session event.' },
  'packages/client/ui-worktree-task': { kind: 'none', reason: 'The browser Worktree Task settings page registers no prompt, tool, or Session event.' },
  'packages/client/ui-workspace-isolation': { kind: 'none', reason: 'The browser Workspace Isolation settings page registers no prompt, tool, or Session event.' },
  'packages/execution-host/execution-host': { kind: 'none', reason: 'The execution-host identity service registers no prompt, tool, or Session event.' },
  'packages/execution-host/execution-host-local': { kind: 'none', reason: 'The local execution-host provider registers no prompt, tool, or Session event.' },
  'packages/security/assessment-scope': { kind: 'indirect', reason: 'The scope service supplies policy to assessment consumers, which own model-facing behavior.' },
  'packages/security/assessment-scope-session': { kind: 'indirect', reason: 'The Session scope provider binds policy for assessment consumers, which own model-facing behavior.' },
  'packages/security/assessment-scope-static': { kind: 'indirect', reason: 'The static scope provider supplies policy to assessment consumers, which own model-facing behavior.' },
  'packages/security/finding': { kind: 'indirect', reason: 'The finding service supplies durable state to finding tools, which own model-facing schemas.' },
  'packages/security/finding-session': { kind: 'indirect', reason: 'The Session finding provider supplies state to finding tools, which own model-facing schemas.' },
  'packages/security/security-skills': { kind: 'indirect', reason: 'The skill provider supplies entries to the skill consumer, which owns model-facing rendering.' },
  'packages/security/vuln-kb-nvd': { kind: 'indirect', reason: 'The NVD provider supplies records to the vulnerability query consumer, which owns model-facing behavior.' },
