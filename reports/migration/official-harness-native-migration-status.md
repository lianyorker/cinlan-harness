...
| 4 SSH／手机配对 | SSH 远端已完成实证；手机为浏览器配对完成，真机仍未验收 | 验收所用 committed main `8620c09a49` 的 Windows-to-Debian SSH acceptance 已通过任务拥有的 Debian 12 QEMU peer，运行时清单 `5f09e90fe9c37153c634d762d88108a5725b63832d5bd08a141ecd6ccc3417ec` 的 sourceCommit 为同一提交；验收覆盖运行时安装与 digest、目标 revision、Workspace／Session 绑定、远端文件／Subprocess／Bash／read-only Landlock／Git／Terminal／Agent PTC、取消、Session Controller 热重载后的冷恢复及清理。手机配对的真实 HTTPS/WSS Chromium 390×844 流程 2/2 通过，覆盖授权 Session 隔离、刷新、停止生成、正式 ask_user_question、一次性受限写入、持久化与撤销；原生 ADB Loader fixture 40 项通过，但没有物理设备或原生手机安装包证据。 |
| 5 剩余功能／安装包 | committed-main EXE/MSI 构建已完成；签名与安装运行验收未宣称 | SDK 最小 profile 环境说明已修正并经真实进程请求验证；Plugin Manager 与 HMR 统一生命周期已通过既有回归；终端标题／恢复已通过真实 Chromium 刷新和最终回放。验收与打包所用 committed main `8620c09a49` 的官方构建与 `node --import tsx/esm apps/desktop/scripts/package-target.ts win-x64` 均退出 0；安装器未执行，签名状态为 NotSigned。 |

## 本轮可复核证据

完整 SSH 验收由任务拥有的单进程 fixture runner 执行：runner 启动 Debian 12 QEMU、通过 OpenSSH 传入已校验的 Node v24.9.0、安装真实 Git 2.39.5，随后运行 `$env:DSH_WINDOWS_LINUX_SSH_SCENARIO=(Resolve-Path .artifacts/native-migration/windows-linux-ssh-scenario.json).Path; $env:DSH_SNAPSHOT="replay"; node node_modules/vitest/vitest.mjs run --config vitest.web.config.ts apps/web/tests/windows-linux-ssh.acceptance.e2e.ts`。该 committed main 的官方 Loader 组合在真实 Windows-to-Debian peer 上通过 1 个测试，Vitest 报告测试 68.17 秒、总时长 70.51 秒；运行目录 `.artifacts/native-migration/posix-fixture/run-14849aa4-5fba-4b48-ad1d-5d6fed23f8b9/closed.json` 已记录关闭。该验收不是目录检查，也没有执行 Windows 安装器。
...
官方构建脚本在提供仓库要求的 pnpm lifecycle 环境后成功完成，client build record 为 `8620c09`、`287` 个 client artifact、SHA-256 `634750d55a54f5ca4571b5fd508a7fad23835a987445e453c97987e16b9fe54a`；构建输出有既有 Vite chunk-size warning，不是失败。构建后运行同一 SSH acceptance 仍通过。

Committed main 的 Windows packaging 命令 `node --import tsx/esm apps/desktop/scripts/package-target.ts win-x64` 退出 0。EXE 为 447,505,079 字节，SHA-256 `9e78253bcf2ebede1d848d624423f1b8101ecb822569d0e2af5e4edbcf4f87be`；MSI 为 474,927,844 字节，SHA-256 `a3e0c6c7428efca559f7c110c884e2ab2c9bcb5715b24d4725e067814838942d`；blockmap 为 465,363 字节，SHA-256 `dcb2d4229701998b5369872798a25406562abb5d60106ec3ef7a7ecddface75a`。生成的 `nightly.yml` 与 EXE 大小及 SHA-512 一致。`Get-AuthenticodeSignature` 报告 EXE 与 MSI 均为 `NotSigned`。没有执行 EXE/MSI，也没有宣称签名或安装后运行验收。
...
文档 quick gate 仍被基线债务阻断：旧 docs/dev 双语缺失、既有 Markdown wrap、README Summary／Model Experience／Limitations 失败以及既有 doc-standard 失败；本轮新增和修改文档的 Agent Note format、classification、三组 bilingual pairing 定向检查均通过。恢复的用户 website WIP 使 `project-doc-site.spec.ts` 出现 68 passed、1 failed：断言要求的 `browser-use.md`、`ptc-runtime.md`、`ssh.md` subsystem README 行尚未存在；主题修复后的 website build 与 3,096 个内部 fragment 校验通过。完整全库 lint 的既有失败也未被改写为通过。
