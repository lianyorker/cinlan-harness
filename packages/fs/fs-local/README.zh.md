...
加载后端并给出基准目录；相对路径以它为基准解析，绝对路径忽略它。相对基准目录以提供方进程的工作目录为锚点，显示路径始终为绝对路径。在 POSIX 上，`symlink/..` 到达链接目标的父目录，即使最终文件尚不存在也如此；通过缺失目录遍历 `..` 会以 `FS_NOT_FOUND` 失败。目录列表与 `lstat` 保留相同的物理遍历语义。Windows 保留原生的驱动器相对路径规范化。
