 ...
 * otherwise preserve the session cwd and leave executor defaulting as the
 * fallback. The policy root and workdir use the same execution-world spelling;
 * the provider resolves symlinks and parent components on its own host.
 */
function resolveWorkdir(
  ...
  if (sessionCwd !== undefined && !isAbsolute(modelWorkdir)) {
    const separator = sessionCwd.startsWith('/') ? '/' : sep
    return `${sessionCwd}${separator}${modelWorkdir}`
  }
  return modelWorkdir
}
