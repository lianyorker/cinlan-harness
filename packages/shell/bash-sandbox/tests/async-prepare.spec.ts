...
async function setup(dialect: 'bash' | 'pwsh') {
  ...
  if (dialect === 'bash') await ctx.plugin(SandboxBashExecutor, { timeoutMs: 100, graceMs: 100 })
  else await ctx.plugin(SandboxPwshExecutor, { timeoutMs: 100, graceMs: 100 })
  ...
  const background = (signal?: AbortSignal) => {
    const promise = ctx.shell.start(ctx.shell.resolve({ command: 'fixture background', signal }))
    runs.push(promise)
    return promise
  }
  return { ctx, prepared, entered, spawned, completion, rangeExit, joined, waitForExit, wrap, confine, spawn, terminate, start, background }
}

describe.each(['bash', 'pwsh'] as const)('%s asynchronous preparation', (dialect) => {
  it.each(['success', 'rejection'] as const)('times out unresolved preparation and prevents late %s from spawning', async (late) => {
    const test = await setup(dialect)
  ...
  it.each(['timeout', 'caller'] as const)('awaits the managed range after %s cancellation rejects before native launch', async (cause) => {
    const test = await setup(dialect)
    const controller = new AbortController()
    const run = test.start(10, controller.signal)
    await test.entered.promise
    ...
    expect(run.observed.done).toBe(false)
    test.rangeExit.resolve(true)
    expect(await run.promise).toMatchObject({
      exitCode: null, signal: null, timedOut: cause === 'timeout', aborted: cause === 'caller',
    })
  })

  it('propagates managed-range observation failure after cancellation', async () => {
    const test = await setup(dialect)
  ...
  it('keeps infrastructure failures visible when they race cancellation', async () => {
    const test = await setup(dialect)
    const run = test.start(10)
    await test.entered.promise
    test.prepared.resolve(test.wrap)
  ...
  it('publishes a background handle only after confinement and stamps immediately settled results', async () => {
    const test = await setup(dialect)
    const pending = test.background()
    await test.entered.promise
    expect(test.spawn).not.toHaveBeenCalled()
    test.completion.resolve({ exitCode: 3, signal: null })
    test.prepared.resolve(test.wrap)
    const process = await pending
    await process.done
    expect(test.spawn).toHaveBeenCalledOnce()
    expect(process).toMatchObject({ exitCode: 3, status: 'completed', sandbox: { mode: 'read-only', denied: false, enforcement: 'full' } })
  })

  it.each(['exit', 'observation failure'] as const)('joins background provider failure until managed-range %s', async (outcome) => {
    const test = await setup(dialect)
    const pending = test.background()
    await test.entered.promise
    test.prepared.resolve(test.wrap)
    const process = await pending
    let settled = false
    const observed = process.done.then(() => { settled = true }, (error: unknown) => { settled = true; return error })
    test.completion.reject(new Error('provider rejected during native startup'))
    await test.joined.promise
    ...
    if (outcome === 'exit') {
      ...
      expect(process.readOutput().delta).toContain('provider rejected during native startup')
    } else {
      const failure = new Error('background managed range observation failed')
      test.rangeExit.reject(failure)
      await expect(process.done).rejects.toBe(failure)
    }
    await observed
  })

  it('prevents a late background preparation from spawning after caller cancellation', async () => {
    const test = await setup(dialect)
    const controller = new AbortController()
    const pending = test.background(controller.signal)
    expect(await test.entered.promise).toBe(controller.signal)
    const reason = new Error('cancel background preparation')
    controller.abort(reason)
    test.prepared.resolve(test.wrap)
