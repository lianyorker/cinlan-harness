...
本包固定使用 `@playwright/mcp` 0.0.80，并依赖共享浏览器运行时。Node 版本须满足 Harness 范围（`^22.19.0 || >=24.0.0`）；固定版本 MCP 包要求 Node >=18。在 Windows 上，通过 `executablePath` 指定已安装的 Chromium `.exe`，或提供外部管理浏览器的调试端点。提供方用当前 Node 可执行程序运行已安装的 MCP JavaScript 入口，不使用 shell，也不要求全局安装 MCP。Windows 移植通过无需密钥的模拟测试验证；真实浏览器连接仍须由操作者验证。
