...
本包固定使用 `@browserbasehq/stagehand` 4.1.0 和 `@puppeteer/browsers` 3.2.2，并依赖 `zod` ^4.4.3。前两者的最低 Node 版本分别为 22.18.0 和 22.12.0；须使用 Harness 范围（`^22.19.0 || >=24.0.0`）。在 Windows 上，`executablePath` 指向已安装的 `.exe`；清理启动的进程需要有权通过 `taskkill` 终止其进程树。内置 Stagehand 扩展声明最低 Chrome 版本为 116，但自动加载扩展还要求浏览器提供 CDP 方法 `Extensions.loadUnpacked`。仅满足版本下限不能证明兼容性。如果该方法不可用，请使用兼容浏览器，或连接操作者已安装扩展的浏览器并设置 `extensionId`。提供方不会添加通用的实验性 API 标志。Windows 模拟测试不能证明特定浏览器构建或模型账户已配置。
...
现有 Chromium 浏览器必须开放 CDP，并允许 Stagehand 扩展连接。提供方的启动器使用 `--remote-debugging-port=0`、`--remote-allow-origins=*`、`--enable-unsafe-extension-debugging` 以及独立的 `--user-data-dir`。对于外部启动的浏览器，须单独配置这些要求，并将 `cdpEndpoint` 设为 Chrome 报告的端点。
...
[`stagehand_` 工具注册](src/index.ts)根据 SDK 方法 schema 提供导航、标签页管理、截图、操作、观察和提取。结果包含当前页面事实或验证后的结构化数据。支持的截图以持久图像附件呈现。错误保持可见，让模型在重试前检查状态。
