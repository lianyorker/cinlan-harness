...
The package pins `@browserbasehq/stagehand` 4.1.0 and `@puppeteer/browsers` 3.2.2, with `zod` ^4.4.3. Their Node floors are 22.18.0 and 22.12.0; use the Harness range (`^22.19.0 || >=24.0.0`). On Windows, `executablePath` names an installed `.exe`, and launched-process cleanup requires permission to terminate its process tree with `taskkill`. The bundled Stagehand extension declares Chrome 116 as its minimum, but automatic extension loading additionally requires the browser CDP method `Extensions.loadUnpacked`. That version floor alone does not establish compatibility. If the method is unavailable, use a compatible browser or attach to an operator-configured browser with the extension already installed and set `extensionId`. The provider adds no general experimental-API flag. Windows mock tests do not establish that a particular browser build or model account is configured.
...
An existing Chromium browser must expose CDP and permit the Stagehand extension to connect to it. The provider's launcher uses `--remote-debugging-port=0`, `--remote-allow-origins=*`, `--enable-unsafe-extension-debugging`, and a dedicated `--user-data-dir`. For an externally launched browser, configure these requirements separately and set `cdpEndpoint` to Chrome's reported endpoint.
...
The [`stagehand_` tool registration](src/index.ts) exposes navigation, tab management, screenshots, actions, observation, and extraction from the SDK method schemas. Results contain current page facts or validated structured data. Supported screenshots appear as durable image attachments. Errors remain visible so the model can inspect state before retrying.
