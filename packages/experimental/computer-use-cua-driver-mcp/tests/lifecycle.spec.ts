...
vi.mock('@deepseek-ai/dsh-mcp-client', async (importOriginal) => {
  ...
  return {
    ...original,
    launchMcpClient(ctx: Context, config: unknown) {
      fake.configurations.push(config)
      ctx.effect(() => () => fake.close())
      return {
        ready: fake.start().then(() => ({})),
        getSnapshot: () => ({ phase: 'stopped', errorCode: fake.closeTimeout ? 'close-timeout' : undefined }),
      }
    },
  }
})
...
describe('installed Cua Driver ownership', () => {
  ...
  it('retains desktop ownership when the MCP transport cannot confirm shutdown', async () => {
    const ctx = await context()
    const fiber = await ctx.plugin(Provider, {})
    fake.closeTimeout = true
    await fiber.dispose()
    expect(ctx.computerUse.providerName).toBe('cua-driver-mcp')
    expect(() => ctx.computerUse.register(ComputerUseProviderName('native'))).toThrow('already registered')
  })
