...
async function expectActiveDriver(root: string, connectionCount: number): Promise<void> {
  ...
  expect(starts).toHaveLength(connectionCount)
  expect(new Set(starts)).toEqual(new Set(drivers))
  for (const pid of drivers.slice(0, -1)) expectProcessExited(pid)
...
describe('installed Cua Driver Loader composition', () => {
  ...
  it('fails the Loader entry and releases ownership when installed driver initialization fails', async () => {
    await expect(load('fail')).rejects.toThrow('initial connection or tool synchronization failed')
    const ctx = contexts.at(-1)!
    const root = roots.at(-1)!
    expect(ctx.get('computerUse')?.providerName).toBeUndefined()
    expect(ctx.get('tools')?.get(TOOL)).toBeUndefined()
    const events = await driverEvents(root)
