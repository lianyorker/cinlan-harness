...
async function load(exclusive = false, mode?: string, toolCallTimeoutMs?: number, toolOrder?: string[], browserDisabled = false) {
  ...
  await writeFile(configPath, JSON.stringify([...modules.keys()].map(name => ({
    id: name, name,
    ...name === 'browser' ? { disabled: browserDisabled } : {},
    config: name === 'loop' ? { agents: [] } : name === 'prompt' && toolOrder !== undefined ? { toolOrder } : {},
  }))))
...
describe('Session MCP Loader composition', () => {
  it('leaves a disabled browser provider unregistered without starting its process', async () => {
    const { ctx, root, model } = await load(false, undefined, undefined, undefined, true)
    const owner = await ctx.agents.create({ sessionId: SessionId('disabled-browser'), agentOptions: { provider: 'fixture', model: 'fixture' } })
    expect(ctx.browserUse.providerName).toBeUndefined()
    expect(ctx.tools.schemas(owner.agent)).toEqual([])
    owner.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'Continue without browser tools.' }], source: { kind: 'user' } }))
    await owner.agent.whenIdle()
    expect(model.requests).toHaveLength(1)
    expect(JSON.stringify(model.requests)).not.toContain('BROWSER_FIXTURE_INSTRUCTION')
    await expect(readFile(join(root, 'events.ndjson'), 'utf8')).rejects.toMatchObject({ code: 'ENOENT' })
  })
  ...
  it('allows another Session to answer while attached-browser discovery is pending', async () => {
    ...
    await vi.waitFor(async () => { expect((await events(root)).some(event => event.event === 'initialize')).toBe(true) })
    const second = await ctx.agents.create({ sessionId: SessionId('second'), agentOptions: { provider: 'fixture', model: 'fixture' } })
  ...
  it('rolls back failed discovery and stops a child when unload interrupts discovery', async () => {
    ...
    await vi.waitFor(async () => { expect((await events(held.root)).some(event => event.event === 'initialize')).toBe(true) })
  ...
  it('holds an immediate PTC turn until the first SDK includes browser tools', async () => {
    ...
    await owner.agent.whenIdle()
    expect(model.requests, JSON.stringify(owner.agent.session.snapshotEvents())).toHaveLength(1)
  ...
  it('awaits discovery on persisted resume and releases a canceled resume before retry', async () => {
    ...
    await vi.waitFor(async () => { expect((await events(root)).filter(event => event.event === 'initialize')).toHaveLength(2) })
    expect(model.requests).toHaveLength(2)
    ...
    await vi.waitFor(async () => { expect((await events(root)).filter(event => event.event === 'initialize')).toHaveLength(3) })
