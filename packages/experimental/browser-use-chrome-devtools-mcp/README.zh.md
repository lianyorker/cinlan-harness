...
本包固定使用 `chrome-devtools-mcp` 1.9.0，并依赖共享浏览器运行时。Node 版本须满足 Harness 范围（`^22.19.0 || >=24.0.0`），此范围同时满足固定版本服务器要求（`^20.19.0 || ^22.12.0 || >=23`）。在 Windows 上，通过 `executablePath` 指定已安装的 Chrome 或 Chromium `.exe`，或提供其 HTTP(S) 或 WS(S) 调试端点。提供方用当前 Node 可执行程序运行已安装的服务器；不要求全局安装 MCP。Windows 移植通过无需密钥的模拟测试验证；真实浏览器连接仍须由操作者验证。
