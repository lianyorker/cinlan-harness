...
describe('opt-in permission scope', () => {
  it('keeps future sessions on the configured default and refuses unmounted Auto restoration', async () => {
    const { ctx, adapter, auto } = await harness([])
    const fresh = ctx.sessions.create(SessionId('unchanged-default'))
    ...
    const selected = autoSession(ctx, 'stored-auto')
    const seed = selected.session.snapshotEvents()
    await auto.dispose()
    expect(() => { ctx.permissionPresets.set(fresh, AUTO_PRESET) }).toThrow('unknown preset')
    expect(() => ctx.sessions.create(SessionId('restore-unavailable-auto'), { seed }))
      .toThrow('cannot restore preset "auto" without its active integration')
    expect(ctx.sessions.get(SessionId('restore-unavailable-auto'))).toBeUndefined()
    expect(adapter.requests).toHaveLength(0)
  })

  it('keeps an existing downstream denial even when review allows the call', async () => {
    const { ctx } = await harness([decisionChunks('{"risk":"low","decision":"allow"}')])
    const probe = registerProbe(ctx)
    ctx.on('tools/pre-execute', () => Promise.resolve({ kind: 'deny', reason: 'owned approval restriction' }))
    const { session, agent } = autoSession(ctx, 'downstream-denial')
    appendHeader(session, [{ name: 'probe', description: 'probe', parameters: { type: 'object' } }])
    const callId = ToolCallId('downstream-denial-call')
    appendAssistant(session, [{ type: 'tool-call', id: callId, name: 'probe', arguments: '{}' }])
    appendNativeCall(session, callId, 'probe', '{}')
    const result = await ctx.tools.execute({ signal: new AbortController().signal, callId, name: 'probe', arguments: {}, agent })
    expect(result).toMatchObject({ isError: true, error: { message: 'owned approval restriction' } })
