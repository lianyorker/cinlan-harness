...
Plugins and hosts create agents through `ctx.agents.create()` and resume persisted sessions through `ctx.agents.resume()`; both return an `AgentHandle` whose `dispose()` owns exact teardown. The loop runs each created agent to completion — the handle is only needed when the caller must tear the agent down itself. Creation and resume flush setup events before invoking the [setup commit](../agent/README.md#create-or-resume-an-agent); its synchronous validation and registry insertion share one publication step.
...
Creation is one rollback-covered transaction: construct a private session, concrete agent, and scoped context; await optional setup; enter both registries; announce `session/created`; await serial `agent/created` initialization; emit `agent/session-start`; then release queued input. A caller creating a runtime child sets `options.parentAgent`; the caller Context separately owns the transaction and live handle. Setup or commit failure leaves both ids unpublished. Creation-listener failure or cancellation rolls back published entries and pairs every begun creation announcement with disposal. Caller and factory teardown signal cancellation, await pending initialization, stop and drain the driver, unwind the scope, close the session write path, then detach the exact agent and session entries. This retains initialization resources through listener settlement and makes ids reusable only after cleanup.
