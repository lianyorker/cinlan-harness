...
创建是同一个受回滚保护的事务：构造私有会话、具象 agent 与带作用域上下文；等待可选 setup；进入两个注册表；宣告 `session/created`；等待串行 `agent/created` 初始化；发出 `agent/session-start`；然后释放排队输入。创建运行时子 Agent 的调用方设置 `options.parentAgent`；调用方 Context 则单独拥有事务和存活 handle。Setup 或 commit 失败时两个 id 均未发布。创建监听器失败或取消会回滚已发布条目，并为每个已开始的创建宣告配对 disposal。调用方与工厂拆除会发出取消信号，等待待完成初始化，停止并排空驱动器，撤销作用域，关闭会话写路径，最后 detach 确切的 agent 与会话条目。这样可将初始化资源保留到监听器完成，并仅在清理后允许复用 id。
