...
The `sidebarTerminals` Remote namespace exposes the [service operations](../../terminal/sidebar-terminals/src/index.ts). Keep the `open` stream alive while rendering a terminal, and send its attachment identity with input, resize, acknowledgement, and release requests. Acknowledgements identify rendered output sequences, not received bytes. The provider owns process generations and release dispositions. Agent `open` targets and `closeAgent` requests both carry `{ sessionId, uuid }`; the provider verifies that ownership before attaching or closing. `inspectUi` returns the existing UI process identity without spawning or extending its lifetime; `closeUi` compares that observed identity and refuses a replacement process.

`listUi(sessionId)` returns retained live UI terminals without spawning. `renameUi` requires Session, tab, process identity, and a nonempty title of at most 120 characters without control characters. The provider trims the accepted title and rejects a replaced process with `sidebarTerminals/stale-attachment`. Client callbacks return the committed snapshot and remain tracked through transport disposal.

`shells(sessionId)` and the Client callback `terminalShells(sessionId)` asynchronously return installed executable paths and names from that Session’s captured execution world without creating a process. UI targets may include `shellPath`, a nonempty, control-free string of at most 4096 characters. The provider accepts only currently discovered choices when spawning; omission selects the local Settings default or remote provider default, while reattachment preserves a live process. Input and resize await the selected process provider. Controller disposal rejects any unsettled shell discovery, input, or resize call after its provider settles, and waits for that settlement before completing. Agent targets reject shell selection. An unavailable explicit choice reports `sidebarTerminals/invalid-shell`.
...
The controller registers asynchronous unary calls before provider entry and checks its Cordis lifetime before entry and after settlement. It combines stream cancellation with the same lifetime. Disposal aborts the lifetime and awaits tracked unary calls plus provider iterator cleanup even when a client pauses iteration. Removing the Loader entry withdraws Remote discovery. No invariant companion is published because the controller owns no independently observable terminal state to compare with the provider; operation quiescence and iterator cleanup are lifecycle obligations covered by composition tests.
...
- Provider iterators must cooperate with abort signals and settle their cleanup; asynchronous shell discovery, input, and resize calls must also settle. Controller disposal awaits both kinds of work instead of abandoning them.
