...
`sidebarTerminals` Remote 命名空间公开[服务操作](../../terminal/sidebar-terminals/src/index.ts)。渲染终端时保持 `open` 流存活，并在输入、尺寸调整、确认和释放请求中携带其连接标识。确认值表示已渲染的输出序列号，而不是已接收的字节数。提供方负责进程代次和释放方式。代理 `open` 目标与 `closeAgent` 请求都携带 `{ sessionId, uuid }`；提供方在连接或关闭前验证所有权。`inspectUi` 返回已有界面进程身份，不创建进程或延长生命周期；`closeUi` 比较已观察到的身份，拒绝关闭替代进程。
...
`shells(sessionId)` 和客户端回调 `terminalShells(sessionId)` 异步返回该 Session 已捕获执行环境中安装的可执行文件路径与名称，不创建进程。界面目标可以包含 `shellPath`，其值必须是非空、不含控制字符且最多 4096 个字符的字符串。提供方仅在创建进程时接受当前发现的选项；省略时选择本地设置默认值或远程提供方默认值，重新连接时保留存活进程。输入和调整尺寸操作等待所选进程提供方完成。控制器释放会在提供方完成后以取消拒绝尚未完成的 Shell 发现、输入或尺寸调整调用，并在这些调用完成后再结束。代理目标拒绝 Shell 选择。显式选择不可用时报告 `sidebarTerminals/invalid-shell`。
...
控制器在进入提供方前登记异步一元调用，并在进入前和完成后检查其 Cordis 生命周期。它将流取消信号与同一生命周期合并。释放会中止该生命周期，并等待已登记的一元调用及提供方迭代器清理，即使客户端已暂停迭代。移除 Loader 条目会撤销 Remote 发现。控制器不发布 invariant 配套入口，因为它没有可独立观测并与提供方比较的终端状态；操作静止和迭代器清理是由组合测试覆盖的生命周期义务。
...
- 提供方迭代器必须响应取消信号并完成清理；异步 Shell 发现、输入与尺寸调整调用也必须完成。控制器释放会等待这两类工作，不会将其遗弃。
