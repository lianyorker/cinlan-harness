...
Client consumers can keep a catalog child open beside the selected Session with `ctx.sessions.retainSubagent(address, { signal })` after `refreshSubagents(parentSessionId)` resolves. The returned reference makes `binding(reference.sessionId)` available immediately; `ready` settles after the shared initial history attempt, with business failures exposed in the Session snapshot. Retaining never changes selection. Each reference owns its full lifetime: `release()` or signal cancellation releases only that reference, and pending readiness rejects on release or scope disposal. The last release tears down an unlisted child once it is no longer selected; listed Sessions and other references keep their scope and follow.
