...
Every Host invocation and stream open carries an explicit authenticated `HostConnectionAccess`. Local HTTP and WebSocket carriers share their Connection instance's trusted identity. Delegated identities require a registered `TypertGatewayAccessPolicy`; unknown identities fail before lookup, and revocation cancels the retained stream lifetime. Policies project aggregate results and stream items, filter event delivery before queueing, bound delegated event queues, and bind interaction responses to the authenticated event client. `currentAccess()` is available only during an authenticated unary dispatch for local-only management executors.
...
- Trusted-local forwarded events reach `$on` without business-payload projection or redaction; delegated carriers apply their registered visibility and item policies. Ordinary notifications are not replayed after reconnect; Agent-scoped waterfalls project only the top-level Agent identity needed to select the Client Context and carry their own pending lifetime.
