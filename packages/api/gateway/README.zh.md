...
每个 Host 调用和流打开操作都携带显式已认证的 `HostConnectionAccess`。本地 HTTP 与 WebSocket 传输共享其 Connection 实例的可信身份。委托身份必须注册 `TypertGatewayAccessPolicy`；未知身份在 lookup 前被拒绝，撤销会取消所保留流的生命周期。策略裁剪汇总结果及流项，在排队前过滤事件投递，限制委托事件队列，并把交互回答绑定到已认证事件客户端。`currentAccess()` 仅在已认证的一元分发期间为仅限本地的管理执行者提供调用者。
...
- 可信本地转发事件到达 `$on` 时不做业务载荷投影或脱敏；委托传输应用其注册的可见性与流项策略。普通通知在重连后不重放；Agent-scoped waterfall 只投影选择 Client Context 所需的顶层 Agent 身份，并自行携带 pending 生命周期。
