...
if ([string]::IsNullOrWhiteSpace($KeystorePath)) {
    $KeystorePath = Join-Path ([System.IO.Path]::GetTempPath()) ("dsh-apk-debug-{0}.keystore" -f [System.Guid]::NewGuid().ToString('N'))
}
...
function Get-ToolPath {
    ...
    $manualHint = switch ($Name) {
        'adb'       { "After explicit approval run: powershell -File `"$bootstrapScript`" -Capability adb -Install" }
        'apktool'   { "After explicit approval run: powershell -File `"$bootstrapScript`" -Capability apktool -Install" }
