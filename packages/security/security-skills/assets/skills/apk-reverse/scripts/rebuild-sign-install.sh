...
DEFAULT_KEYSTORE="${TMPDIR:-/tmp}/dsh-apk-debug-$$.keystore"
...
if [[ -z "$PROJECT_DIR" || ! -d "$PROJECT_DIR" ]]; then
    ...
    echo "  --keystore <path>  签名密钥库（默认: 临时 debug keystore）"
...
ensure_tool() {
    ...
    echo "ERR: $name 未找到。请先按 skill 的按需自举说明，在取得明确授权后运行 bootstrap。" >&2
    if ! command -v "$name" &>/dev/null; then
        echo "ERR: $name 不可用。"
        case "$name" in
