...
if (-not $SkipJadx) {
    ...
    if (-not $jadxSpec.Available) {
        throw "Missing required CLI tool: jadx. After explicit approval run: powershell -File `"$bootstrapScript`" -Capability jadx -Install"
    }
}
if (-not $SkipApktool) {
    $apktoolSpec = Resolve-ReverseToolSpec -Name 'apktool'
    if (-not $apktoolSpec.Available) {
        throw "Missing required CLI tool: apktool. After explicit approval run: powershell -File `"$bootstrapScript`" -Capability apktool -Install"
    }
}
