...
- 未提供 `-KeystorePath`/`--keystore` 时在临时目录生成一次性调试 keystore；需要复用签名身份时必须显式传入 keystore 路径
...
缺少工具时入口脚本会报告具体缺失项，不会自动安装。取得明确授权后，使用 bootstrap 的 `-Install` 模式。
...
- `scripts/decode.ps1`：缺 jadx 或 apktool 时输出授权后可运行的安装命令
- `scripts/rebuild-sign-install.ps1`：缺 adb 或 apktool 时输出安装命令
- `scripts/frida-run.ps1`：缺 frida 时输出安装命令

### 自举失败时

显式运行 bootstrap 安装失败时会抛出错误。常见原因：
