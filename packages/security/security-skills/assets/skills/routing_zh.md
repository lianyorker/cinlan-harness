...
按目标类型、用户意图和工具链，将任务路由到最合适的技能模块。

## 路由判定协议

1. 根据目标类型、用户意图和已知工具证据选择一个主模块。
2. 行动前读取主模块的 `SKILL.md`。
3. 任务确实跨模块时再加载下一个模块。
4. 路由未命中时报告覆盖缺口，并遵守当前分析/执行意图；不要硬塞或自动新增 skill。
...
| HTTP 抓包 / 浏览器采样 / 请求重放  | `browser-automation/`                                                             | `js-reverse/`、`api-security/` 或可用的抓包 MCP                                                                   |
...
| Go / Rust 二进制                   | `reverse-engineering/languages-compiled.md` + `reverse-engineering/go-reverse.md` | `ida-reverse/` 或 `radare2/`                                                                                      |
| **CTF 竞赛**                       | 按主导证据面路由到现有模块                                                        | `pentest-tools/`、`reverse-engineering/`、`pwn-chain/`、`llm-security/`、`mobile-reverse/` 或 `firmware-pentest/` |
| Web 运行时 / API                   | `api-security/`                                                                   | `pentest-tools/` 或 `browser-automation/`                                                                         |
| 云 / 容器 / K8s                    | `attack-chain/` 做路径规划                                                        | `supply-chain-security/` 做镜像和流水线分析                                                                       |
| Windows / AD / 身份                | `pentest-tools/`                                                                  | `attack-chain/` 做多阶段编排                                                                                      |
| 取证 / PCAP                        | 按证据选择 `malware-analysis/` 或 `pentest-tools/`                                | 隐写未命中时报告覆盖缺口                                                                                          |
| Prompt 注入 / Agent                | `llm-security/`                                                                   | —                                                                                                                 |
| 移动端 (Android/iOS)               | `mobile-reverse/`                                                                 | Android 静态分析用 `apk-reverse/`                                                                                 |
| 固件 / 恶意样本                    | `firmware-pentest/` 或 `malware-analysis/`                                        | `reverse-engineering/`                                                                                            |
| **LLM 应用 / AI Agent**            | `llm-security/SKILL.md` — OWASP LLM + ASI Top 10                                  | —                                                                                                                 |
...
| "Go 逆向/Rust 逆向/stripped"                              | `reverse-engineering/languages-compiled.md` + `reverse-engineering/go-reverse.md`                            |
...
| "云安全/容器逃逸/K8s"                                     | `attack-chain/SKILL.md` 做路径规划 + `supply-chain-security/SKILL.md` 做镜像/流水线分析                      |
| "Prompt 注入/AI 安全"                                     | `llm-security/SKILL.md` — OWASP LLM + ASI Top 10 完整框架                                                    |
| "Agent 不干活/只读不干/跳过步骤"                          | `llm-security/references/agent-obedience-engineering.md` — 仅作提示词行为研究参考，不作为运行时规则          |
| "Prompt 工程/Skill 工程/Harness 工程"                     | `llm-security/references/agent-obedience-engineering.md` — 仅作设计参考                                      |
| "先例/操作记录/做过什么/已执行"                           | 只读取当前任务显式提供的案例或 Session 证据；产品包不内置项目案例历史                                        |
| "授权/是否允许/法律风险/scope"                            | 不按安全主题拒绝或复核授权；实际 effect 由当前请求与 runtime 控制决定                                        |
...
| "脱敏/占位符/分享 payload/写 writeup 前脱敏"              | `templates/security-report-templates.md` — 报告占位符与证据表达                                            |
...
| 工具                                                                      | 相关模块                                                                                                                                        |
| ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| IDA Pro (idapro\_\*)                                                      | `ida-reverse/` — MCP HTTP 服务器 + 72 工具                                                                                                      |
...
若 `tool-index.md` 已生成，可作为缓存并对关键工具做直接复核；索引不存在时用实际命令探测，不猜路径。
...
2. 如果确实是全新类型，报告覆盖缺口，并可提议新增 skill：
   - 说明建议的 skill 名称和覆盖场景
   - 说明需要的工具链
   - 说明与现有 skill 的关系
3. 只有取得执行授权后，才按 `CONTRIBUTING.md` 新增或修改 skill
4. 新增完成后更新本路由矩阵

路由失败是需要报告的证据，不是修改文件或安装依赖的授权。
...
CTF 竞赛路径：
  判断主导证据面（Web / 二进制 / Pwn / 移动 / 固件 / LLM）
  ↓
  api-security 或 pentest-tools / reverse-engineering / pwn-chain /
