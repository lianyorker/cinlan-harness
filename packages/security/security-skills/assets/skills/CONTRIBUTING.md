...
<!-- 哪个脚本会检测该工具，以及授权后可运行的 bootstrap 命令 -->
...
2. `NOW`：若 `../tool-index.md` 存在则读取；否则用实际命令探测工具，不要猜路径。
3. `NEXT`：缺工具时报告缺失项和安装方式；取得明确授权后才调用 bootstrap 的 `-Install` 模式。
4. `ACT`：进入“工作流”第一步并执行，不要停在确认状态。

## 任务完成自检（声称完成前 MUST 通过）

- □ 我是否执行了工作流中的每一步（而不是只阅读）？
- □ 我是否通过本机探测或已生成的 `tool-index` 使用了真实工具路径？
- □ 我是否产出了可复现证据（命令/脚本/截图/报告）？
- □ 我是否遵守当前任务的分析/执行意图边界？

### 3.2 内容策略与学习记录

新 skill 不得按安全主题添加拒绝、警告或授权复核。只有当前任务显式提供案例记录时才读取它们；产品包不提供案例历史目录。实际 effect 仍由 sandbox、permission、approval、credential 和资源控制约束。

新 skill 复用根 router 的学习闭环，不另写一套回写规则。执行任务产生匹配 KB 尚未覆盖、具备证据且可复用的增量时，当前意图允许写 workspace 才按 `templates/knowledge-candidate.md` 创建 `.agents/field-journal/cyber-security/YYYY-MM-DD_<slug>.md`。实例化时不复制模板的语言切换行；目标路径已存在时追加采用 UTC 的 `_HHmmssZ` 与必要的最小未使用序号，绝不覆盖。Candidate 必须脱敏并记录适用版本、证据、限制和建议合入位置；不得自动 commit、push、上传或修改已安装 bundle。用户手工复制或归档该目录后，由维护者复核、去重并验证，才把内容合入 package 源码。
...
脚本中检测工具缺失时，先给出可执行安装命令。只有调用方已获得明确授权时才运行：
...
if (-not $spec.Available) {
    throw "<tool> is missing. After explicit approval run: powershell -File `"$bootstrapScript`" -Capability '<tool-name>' -Install"
}
```
