...
function clearLocalStorage(bg2csData = '') {
  let excluded_domains = ['britannica.com', 'nationalreview.com', 'thecritic.co.uk'].concat(
    usa_mcc_domains
  )

  if (bg2csData && bg2csData.cs_clear_lclstrg && !matchDomain(excluded_domains)) {
    // 全清
...
cs_default_bloomberg = function (bg2csData) {
  // 移除 paywall overlay
  document
    .querySelectorAll('.paywall, #graphics-paywall, .lede-paywall')
  ...
  // 显示被隐藏的段落
  document.querySelectorAll('.body-content p, .article-body p').forEach((el) => {
...
cs_default_wsj = function (bg2csData) {
  ...
  document.querySelectorAll('[class*="snippet"], [class*="truncated"]').forEach((el) => {
    el.classList.remove(
      ...Array.from(el.classList).filter((c) => c.includes('snippet') || c.includes('truncated'))
    )
  })

  document.querySelectorAll('.wsj-snippet-login, .wsj-snippet-pitch').forEach((el) => el.remove())
}
...
cs_default_bi = function (bg2csData) {
  ...
  // 恢复文章高度
  document.querySelectorAll('.article-content, .post-content').forEach((el) => {
    el.style.maxHeight = 'none'
...
for (let n = 0; n < 5; n++) {
  setTimeout(function () {
    ...
    if (!bg2csData.cs_block && typeof cs_default === 'function') cs_default(bg2csData)
...
if (
  matchUrlDomain(
    ['businesspost.ie', 'hbr.org', 'la-croix.com', 'ouest-france.fr'].concat(
      grouped_sites['###_fr_groupe_ebra']
    ),
    url
  )
) {
