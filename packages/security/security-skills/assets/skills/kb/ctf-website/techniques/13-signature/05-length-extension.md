...
related_articles:
  ['ctf-website/13-signature/00-overview', 'ctf-website/13-signature/02-implementation']
---
...
| 场景        | 线索                                       | 攻击要点              | | ----------- | ------------------------------------------ | --------------------- | ------------- | | 支付回调    | `sign=MD5(key+order_id+amount)`            | 扩展 status=paid      | | API 认证    | `X-Sign: SHA256(apisecret+timestamp+path)` | 扩展 &admin=true      | | 文件签名    | 下载页面提供 MD5 签名                      | 文件尾部追加 payload  | | Cookie 签名 | 格式 `data                                 | sign`                 | 检查是否 HMAC | | URL 签名    | `?token=MD5(secret+path)`                  | 扩展路径参数          | | JWT 类似    | 自实现 signature                           | 检查算法是否为裸 hash |
