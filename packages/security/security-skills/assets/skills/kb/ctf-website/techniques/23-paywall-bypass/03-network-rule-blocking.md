...
function regexToUrlFilter(rule, regex, domain) {
  ...
  if (!(regex.match(/([([|*{$\^]|\\[a-z\?])/) || regex.match(/([^\.]|\\\.)\+/))) {
    let match_domain = gpw_domains
      .concat(['tinypass.com', 'wallkit.net', domain])
      .find((x) => regex.replace(/\\/g, '').match(new RegExp(x.replace(/\./, '\\.'))))

    urlFilter = regex.replace(/\\/g, '').replace(/\.\+/g, '*')

    if (match_domain) urlFilter = '||' + urlFilter.replace(/^[\.\/]/g, '')
...
function blockJsInlineListener(details) {
  ...
  if (!isSiteEnabled(details) || !matched) return
...
ext_api.webRequest.onHeadersReceived.addListener(
  ...
  { types: ['main_frame', 'sub_frame'], urls: blocked_js_inline_urls },
  ...
  window.setTimeout(function () {
    ...
    if (amp_page_hide) {
      ...
      function amp_unhide_subscr_section() {
        document
          .querySelectorAll('[subscriptions-section="content-not-granted"]')
      ...
      function amp_unhide_access_hide() {
        document
          .querySelectorAll('[amp-access-hide]')
          .forEach((el) => el.removeAttribute('amp-access-hide'))
      }

      // 3. 加载 data-src 中的真实图片 (AMP 懒加载)
      function amp_images_replace() {
        document.querySelectorAll('amp-img img[src], amp-img img[data-src]').forEach((img) => {
      ...
      function amp_iframes_replace() {
        document.querySelectorAll('amp-iframe iframe').forEach((iframe) => {
          if (iframe.getAttribute('data-src')) iframe.src = iframe.getAttribute('data-src')
        })
      }
...
ext_api.webRequest.onBeforeRequest.addListener(
  function (details) {
    if (!isSiteEnabled(details)) return
    ...
    if (domain && details.url.match(blockedRegexes[domain])) return { cancel: true }

    if (matched_general && details.url.match(blockedRegexesGeneral[matched_general].block_regex))
      return { cancel: true }
  },
