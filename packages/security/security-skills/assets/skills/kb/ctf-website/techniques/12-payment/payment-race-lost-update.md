...
### 1.2 必要条件 1. 两个请求最终写入同一用户、钱包、订单或库存记录。 2. 至少一条路径执行非原子的 read-modify-write，或保存包含敏感字段的旧实体快照。 3. 冲突没有被事务、原子 SQL、正确乐观锁重试或串行化队列消除。 4. 攻击者能同时调用计费动作与第二条写路径。 5. 存在可靠状态 oracle：`quota/balance/credits/stock`、账单流水或权益数量。
