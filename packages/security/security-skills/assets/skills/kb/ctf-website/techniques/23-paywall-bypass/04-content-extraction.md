...
function getArticleJsonScript() {
  ...
  for (let script of scripts) {
    try {
      ...
      if (json['@type'] && /article|newsarticle|blogposting/i.test(json['@type'])) return script
    } catch (e) {}
  }
  ...
  for (let script of scripts) {
    try {
      let json = JSON.parse(script.text)
      if (findKeyJson(json, /^articlebody$/i)) return script
    } catch (e) {}
  }
  // 优先级 3: 返回第一个 JSON-LD
  return scripts[0]
    ...
    for (let key in obj) {
      if (typeof obj[key] === 'string') {
        for (let s of search) {
          ...
          if (match && obj[key].length > min_length) return obj[key]
        }
      ...
      if (paywall.length && article) {
        ...
        if (json_script) {
          try {
            ...
            if (query_slug && Array.isArray(query_slug)) query_slug = query_slug.pop()
            let url_next = query_slug || findKeyJson(json, ['slug'])
            if (
              url_next &&
              typeof url_next === 'string' &&
              !decodeURIComponent(window.location.pathname).endsWith(decodeURIComponent(url_next))
            )
            ...
            let json_text = findKeyJson(
              json,
              ['blocks', 'body', 'BodyPlainText', 'content', 'contentHtml', 'description', 'html'],
              500
            ) // 最少 500 字符

            if (typeof json_text === 'string') json_text = parseHtmlEntities(json_text)
...
if (bg2csData.ld_json_source && dompurify_loaded) {
  ...
  if (data.includes('|')) {
    window.setTimeout(function () {
      let [paywall_sel, article_sel, filter_re, json_key, article_append, article_hold] =
        data.split('|')
      let filter = new RegExp(filter_re.replace(/\./g, '\\.').replace('=', '\\s?=\\s?'))
      let paywall = document.querySelectorAll(paywall_sel)
      let article = document.querySelector(article_sel)
      ...
      if (paywall.length && article) {
        ...
        if (json_script) {
          ...
          // 截断到 }; (对象结束)
          if (script_text.includes('};')) script_text = script_text.split('};')[0] + '}'
...
function getSourceJsonScript(filter, not_selector = ':not([src])') {
  ...
  for (let script of scripts) {
    if (filter.test(script.text)) return script
  }
  return null
}
...
if (bg2csData.ld_json_url && dompurify_loaded) {
  ...
  if (data.includes('|')) {
    window.setTimeout(function () {
      let [paywall_sel, article_sel, article_append, article_hold, article_id_sel, key, url_rest] =
        data.split('|')
      ...
      function setMediaSrc(elem) {
        if (elem.getAttribute('data-src')) elem.src = elem.getAttribute('data-src')
        else {
      ...
      func_post = function () {
        document
          .querySelectorAll('figure img[src^="data:image/"], picture img[src^="data:image/"]')
          .forEach(setMediaSrc)
        document.querySelectorAll('iframe[src="about:blank"]').forEach(setMediaSrc)
      }
      ...
      getJsonUrl(
        ...
        '',
        article_sel,
        { art_append: article_append, art_hold: article_hold, art_style: 'margin: 25px 0px;' },
        article_id,
        key,
        url_rest
      )
