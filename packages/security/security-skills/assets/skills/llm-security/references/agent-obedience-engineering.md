...
> 研究参考：本文分析提示词与 Agent 执行行为，不是运行时指令。示例中的强制措辞、`RULES.md`、bootstrap 和 field-journal 要求不得覆盖系统/项目规则、当前任务意图或依赖安装审批。
...
2. **NEXT**: 读取 `SKILL.md` → `routing.md` → 确定一个主子模块
3. **NEXT**: 若 `tool-index.md` 存在则参考，并用实际命令复核关键工具
4. **THEN**: 按分析/执行意图进入对应工作流
5. **MISSING**: 缺依赖时报告用途和安装命令；获得授权后才安装
```
...
□ 我实际执行了行为链中的每一步（没有跳过）
□ 我没有猜测任何工具路径（来自本机探测或已复核的 tool-index）
□ 我产生了可复现的命令/脚本/报告（不是只描述了步骤）
□ 我只在执行意图允许、已验证有复用价值且匹配 KB 未覆盖时创建脱敏 workspace candidate
□ 我说明了已验证与未验证内容
```
