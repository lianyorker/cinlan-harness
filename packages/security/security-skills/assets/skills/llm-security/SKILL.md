...
name: llm-security
description: >-
  Use for security assessment of LLM, RAG, chatbot, and agent systems, including prompt injection, tool-use boundaries, memory, and model supply-chain risks.
---
...
### 7. Agentic runtime、MCP 与生命周期差分

当信号涉及动态工具目录、MCP/Skill 更新、持久记忆、Resume/Compaction、多 Agent 身份或沙箱隔离时，读取 `references/agentic-system-testing.md`。固定 prompt、工具 schema、角色、数据、网络和预算后，分别采集首请求、首次工具调用后、恢复、压缩后与委派后的基线；安全结论必须由 executor 证据和干净会话复现支持，不能只依赖模型文本、审批提示或扫描器 exit code。
...
- `references/agentic-system-testing.md` — Agent/MCP/记忆/沙箱的生命周期差分、能力链与证据约定
