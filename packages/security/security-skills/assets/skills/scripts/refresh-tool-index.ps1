...
$markdownLines = @(
    ...
    '- 说明: 本表由 `scripts/refresh-tool-index.ps1` 自动生成，供支持本 skill 的客户端确认工具路径。',
    '- 注意: 对于 jshookmcp 这类 MCP server，`yes` 只表示本机具备通过 node/npx 拉起它的条件，不表示它已经在当前 Harness 的 Settings → MCP 中配置、连接并提供工具。',
    '',
