...
describe('approveEscalation', () => {
  ...
  it.each(['read-only', 'workspace-write', 'danger-full-access'] as const)('returns repeated %s without approval', async (mode) => {
    const seen: unknown[] = []
    ...
    await expect(approveEscalation(req({ requestedMode: mode, effectiveMode: mode }), ingredients({
      approver: undefined,
      agent: undefined,
    }))).resolves.toBe(mode)
    expect(seen).toEqual([])
  ...
  it('a narrower or unsupported request fails closed and never asks', async () => {
    const seen: unknown[] = []
