...
- **No read-isolation policy** — `WRITE_RESTRICTED` intersects write accesses, while the LUA token still applies ordinary access checks. Private DACLs granting only SYSTEM, Administrators, and OWNER RIGHTS, such as Python-created Windows temporary directories, can deny access even inside the workspace; PowerShell may then fall back to its installation directory. Workspace grants do not override these checks. Network and process visibility remain available.
