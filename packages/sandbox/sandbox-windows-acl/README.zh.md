...
- **不提供读取隔离策略。** `WRITE_RESTRICTED` 对写访问取交集，LUA token 仍执行常规访问检查。仅向 SYSTEM、Administrators 和 OWNER RIGHTS 授权的私有 DACL（例如 Python 在 Windows 上创建的临时目录）可能拒绝对工作区内部的访问；PowerShell 随后可能回退到安装目录。工作区授权不会覆盖这些检查。网络和进程可见性仍然可用。
