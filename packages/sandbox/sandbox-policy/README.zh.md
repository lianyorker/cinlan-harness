...
| `workspaceRoot` | `process.cwd()` | 无 agent（智能体）调用或没有 cwd 的会话使用的执行世界绝对回退根目录；普通 agent 调用改用会话的不可变 cwd |
...
无效的配置模式或相对 `workspaceRoot` 会在插件加载时被拒绝，因此错误配置会明确失败，而不是静默改变策略。没有 cwd 的会话与无 agent 调用回退到配置的工作区根目录；带已批准显式模式的调用只在该次调用中使用该模式。
...
`resolve({ session, mode })` 返回一份完整的逐调用策略：已批准的显式模式优先于会话最后一条 `sandbox/mode` 事件，后者又优先于部署默认值。会话的不可变 `cwd` 成为工作区根目录；否则使用配置的回退值。解析会原样保留执行世界中的绝对路径，包括 `symlink/..`；实施限制的提供方在文件所在的环境中解析文件系统身份。
...
`sandbox:policy` 贡献说明模式的与具体能力无关的文件操作约定，以及 `workspace-write` 下执行世界中的会话工作区。它不枚举已挂载能力；工具插件保留特定于操作的拒绝与升权引导，批准策略单独贡献给同一份快照，计划引导仍由 `dsh-plan-mode` 的系统段落管理。可选的 `./invariant` 配套组件会拒绝值超出封闭模式词汇的伪造持久 `sandbox/mode` 事件。
...
首次请求和有效策略每次变化时增加一条简洁的持久上下文消息；未变化的请求不增加内容。`workspace-write` 只携带原样保留的执行世界会话工作区路径；平台特定的临时路径会以摘要表述，不会加入依赖主机的字节。
