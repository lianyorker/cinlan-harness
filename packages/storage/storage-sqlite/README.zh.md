...
`dsh-storage-sqlite` 是一个存储后端：把每个已路由单元托管在同一个 SQLite 数据库文件中，每条记录按行存储一份 JSON 文档，注册为后端 `sqlite`。单条记录更新触碰一条数据行及其单元版本戳，这正是它适合高频定点写入的原因。当领域数据变动频繁、或部署偏好单一可查询数据库时选择它；当数据需要以纯文本文件形式可读时选择 JSON 后端。本后端只面向宿主侧：它不贡献提示词、工具或 schema，因此模型与 agent loop（智能体循环）永远不会看到它。
...
当写入频繁且为定点更新时选择它——每个键恰好映射到一行，因此更新一条记录触碰其数据行和单元版本戳，而不是重写整个文件。当人类需要以纯文本文件查看或编辑已存数据时选择 JSON 后端。同步的 `node:sqlite` 驱动会在每次变更事务期间阻塞 JavaScript 线程，这在领域数据规模下可以接受，但高写入率时值得纳入考量。
...
缺失的目录与数据库文件会以仅所有者可访问的权限创建（`0o700`／`0o600`）；已有数据库保持其既有模式。单元接受当前版本或 `compatibleVersions` 明确列出的旧版本，其他所有版本戳以 `version-mismatch` 拒绝。打开和 `loadAll()` 不升级已接受版本戳，也不重写存储值。首次成功变更把记录或全局值变化与当前单元版本戳一起提交。数据库物理布局仍要求精确版本，其他 `PRAGMA user_version` 均被拒绝。失败携带稳定的 `StorageError` 代码，写入 resolve 后即已持久。
...
本后端是在单个 `node:sqlite` 连接之上的文档按行布局；每次变更在同一事务中提交预处理数据语句和单元版本戳。

### 设计理念

- **每行一份文档。** 每个单元表都变成一张物理 STRICT 表 `u_<unit>_<table> (key TEXT PRIMARY KEY, value TEXT)`，其 `value` 列保存记录的 JSON 文本；全局单例存放在共享的 `unit_globals` 表中。一个键的更新触碰一条数据行和单元元数据——这就是把高频变更领域路由到这里的原因。
- **数据与版本共同提交。** `putRecord`、`deleteRecord` 和 `setGlobal` 使用 `BEGIN IMMEDIATE`，重新检查存储版本为当前版本或已接受旧版本，执行变更，更新 `units.version`，然后提交。失败时两项变化一起回滚。其他句柄提交较新版本后，旧打开句柄的写入会被拒绝；领域内写入顺序仍由调用方负责。
- **名称在 DDL 之前校验。** 单元名与表名在进入 DDL 之前必须匹配 `UNIT_NAME_RE`，因此任何外部输入都不会被插值进 SQL 标识符。
- **版本明确报错。** 物理布局版本存放在 `PRAGMA user_version`（全新数据库最后盖戳）；单元格式版本存放在 `units` 表中。`compatibleVersions` 只适用于后者。兼容单元升级不改变物理 DDL，因此 `STORAGE_SQLITE_SCHEMA_VERSION` 保持不变。
...
| — | 不发布运行时不变式伴生入口；打开时检查物理布局，打开及每次变更时检查单元版本戳，持久性由后端往返与回滚测试验证。 |
...
- **同步驱动阻塞事件循环**——每次写入都是一次同步 `DatabaseSync` 事务；阻塞覆盖版本戳检查、数据语句和版本戳更新，在领域数据规模下可以接受。
