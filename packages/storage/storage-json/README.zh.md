...
缺失的 `single` 文件或 `per-record` 目录会作为空单元打开，并在第一次写入时物化。在 `single` 中，畸形内容以 `malformed-medium` 拒绝；当前版本戳和 `compatibleVersions` 明确列出的旧版本之外的版本，以 `version-mismatch` 拒绝。打开和 `loadAll()` 保持已接受前代的文件字节不变。内存状态使用当前版本，首次成功文件发布把该版本戳与数据变化一起写入；发布失败保留前代文件，删除不存在的键不执行发布。在 `per-record` 中，每份畸形或不可读的文档，以及版本不在 descriptor 当前版本和兼容版本内的文档，都读作记录不存在，因此单个坏文档不会使单元被拒绝。记录键必须匹配 `[a-zA-Z0-9_-]+`；不安全的键在任何文件操作前被拒绝。每次已完成的写入都已持久化，关闭后的操作以 `closed` 拒绝。
