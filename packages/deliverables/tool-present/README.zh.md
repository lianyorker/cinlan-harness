...
纯 `./types` 入口声明 `PresentedFile` 与读取时必须识别的 Session 事件，不导入 Host 运行时代码。交付事件不含 `ignorable` 标记；丢失声明会改变重建的交付历史。Web 消费方在展示或打开文件前校验持久声明。事件不保存 Session ID，因此 fork 历史中的相对路径按当前查看的 Session 工作区解析。
...
- [Session 事件规则](../../core/session/README.zh.md)——除非显式标记为可忽略，否则读取方必须识别事件。
