...
describe('syncTools', () => {
  ...
  it('keeps callable tools and committed descriptors after a pagination cycle', async () => {
    const client = createMockClient([{ name: 'stable', inputSchema: { type: 'object' } }])
    const onCommit = vi.fn()
    const opts = { ...defaultOpts, onCommit }
    ...
    expect(onCommit).toHaveBeenLastCalledWith([expect.objectContaining({ name: 'mcp__srv__recovered' })])
  })
