---
description: "打开一个双工作台，处理文件、预览、终端、Git、后台任务、内嵌 Browser 页面和可扩展的插件面板。"
...
打开一个双工作台，处理文件、预览、终端、Git、后台任务、内嵌 Browser 页面和可扩展的插件面板。
...
- [🔐 安全](#-security) · [⚠️ 已知限制与后续工作](#known-limitations-and-deferred-work) · [🖥️ 平台支持](#-platform-support)
- [💬 社区](#-community) · [🤝 参与贡献](#-contributing) · [⭐ Star History](#-star-history) · [🔗 友情链接](#-friends)
...
| 终端提示「node-pty 加载失败」 | `node-pty` 安装缺失/损坏（如 pnpm 拦截了构建脚本）。终端横幅会给出修复命令：复制到 DSH 所在环境的终端/cmd 执行（在 `~/.dsh/profiles/web` 下 `pnpm approve-builds --all && pnpm rebuild node-pty`），完成后重启 DSH 并点重试。插件与 DSH 核心使用同一 `node-pty@1.2.0-beta.15`，修复后两者同步恢复。 |
...
挂载该包后，模型会收到八个终端定义：`terminal_create`、`terminal_list`、`terminal_send`、`terminal_read`、`terminal_wait_for`、`terminal_resize`、`terminal_signal` 与 `terminal_close`。调用操作与当前 Harness Session 关联的持久终端会话；结果报告不透明终端句柄、有界 transcript 分页、等待结果与生命周期确认。

#### Token 影响

启用该包时，八个定义会为每次 request 增加固定 schema 成本。每次调用与结果都会追加普通 tool event；`terminal_read` 将分页限制为 500 行与 256 KiB，保证单次 transcript 读取有界。
...
- 移动端视口不可用桌面底部面板
- 不发布 runtime invariant companion，因为该插件除了 registration 与 route operation 外不拥有可独立观察的关系。
