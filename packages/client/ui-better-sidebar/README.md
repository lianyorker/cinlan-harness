---
description: "Open a dual workbench for files, previews, terminals, Git, background tasks, embedded Browser pages, and extensible plugin panels."
...
Open a dual workbench for files, previews, terminals, Git, background tasks, embedded Browser pages, and extensible plugin panels.
...
- [🔐 Security](#-security) · [⚠️ Known Limitations and Deferred Work](#known-limitations-and-deferred-work) · [🖥️ Platform Support](#-platform-support)
...
Double-click a terminal tab or choose **Rename terminal** from its context menu. The tab title changes only after the Host accepts it. Opening a Session restores tabs for its retained live processes, including their titles and shell choices, even when the Client layout is missing. Recovery failures expose **Retry**. Main and floating windows recover only their own terminals; Host restart does not restore processes, and terminal titles do not enter Session JSONL.

New terminal tabs query installed shells in their Session’s captured execution world before launch. Local Sessions use the Settings default and node-pty; SSH Sessions use the remote default shell and the leased subprocess terminal service. Each tab stores its accepted choice with the layout; selecting a shell does not change Settings. `shellCandidates` controls discovery (default: `zsh`, `bash`, `fish`, `pwsh`, `powershell`, `cmd`). Explicit choices are checked again before spawning, and unavailable choices fail without falling back.

A UI process retains the lease returned by `executionBindings.forSession` until its native exit and provider cleanup settle. Remote PTYs use only that lease’s subprocess service; floating directories use its filesystem and POSIX containment rules. A missing binding, lost lease, or remote failure never selects a local process. Reconnect preserves the captured process, title, directory, and execution incarnation; failed worlds are excluded from retained-list recovery. Host disposal cancels pending allocations and awaits their processes and leases. If process termination cannot be confirmed, cleanup reports failure and retains the lease; it does not report the process as safely released.

The optional sidebar `terminal_*` model tools operate only on local Sessions. Every operation checks the initiating Session’s execution binding before accessing the local agent terminal registry. SSH Sessions use the official execution terminal or Bash capabilities; their interactive UI tabs use the remote lease described above.
...
The Host mounts the terminal provider without requiring a Web server. Local UI and agent PTYs inherit the shared subprocess service's credential-scrubbed parent environment; ambient `DEEPSEEK_*`, `DSH_*`, and credential-shaped variables do not reach their shells. Complete serialized output frames default to 16 KiB, retained output to 8 MiB, acknowledgment timeout to 30 seconds, and native shutdown timeout to 10 seconds; configure `terminalFrameBytes`, `terminalBufferBytes`, `terminalAckTimeoutMs`, and `terminalShutdownTimeoutMs` in the plugin config. The buffer must be at least one frame and includes JSON escaping and frame metadata. Native shutdown waits for every owned process, including unattached and already-closing terminals.
...
pnpm build        # → lib/index.js + lib/client.js + lib/client-registry.js + lib/types
...
When the package is mounted, the model receives eight terminal definitions: `terminal_create`, `terminal_list`, `terminal_send`, `terminal_read`, `terminal_wait_for`, `terminal_resize`, `terminal_signal`, and `terminal_close`. Calls operate on persistent terminal sessions associated with the current Harness Session; results report opaque terminal handles, bounded transcript pages, wait outcomes, and lifecycle acknowledgements.

#### Token effect

The eight definitions add a fixed schema cost to each request while the package is enabled. Each call and result appends ordinary tool events; `terminal_read` limits pages to 500 lines and 256 KiB so one transcript read remains bounded.
...
- The desktop bottom panel is unavailable on mobile viewports
- No runtime invariant companion is published because the plugin owns no independently observable relation beyond its registrations and route operations.
