...
describe('/sidebar/bundle route', () => {
  it('serves an allowlisted chunk with the JS content type and an ETag', async () => {
    ...
    try {
      const res = fakeRes()
      await handler(req('GET', '/sidebar/bundle/editor.js'), res as unknown as SidebarHttpResponse)
  ...
  it('revalidates with a 304 when If-None-Match matches (no re-download)', async () => {
    ...
    try {
      const first = fakeRes()
      await handler(req('GET', '/sidebar/bundle/editor.js'), first as unknown as SidebarHttpResponse)
      const etag = first.headers.etag!
      const second = fakeRes()
      await handler(req('GET', '/sidebar/bundle/editor.js', { 'if-none-match': etag }), second as unknown as SidebarHttpResponse)
  ...
  it('serves the new body after the file changed (ETag rotates)', async () => {
    ...
    try {
      ...
      const second = fakeRes()
      await handler(req('GET', '/sidebar/bundle/editor.js', { 'if-none-match': first.headers.etag! }), second as unknown as SidebarHttpResponse)
  ...
  it('rejects unknown chunk names (including traversal attempts) with 404', async () => {
    ...
    try {
      for (const url of ['/sidebar/bundle/evil.js', '/sidebar/bundle/xlsx', '/sidebar/bundle/../secret.js', '/sidebar/bundle/xlsx.js/extra']) {
        const res = fakeRes()
        await handler(req('GET', url), res as unknown as SidebarHttpResponse)
  ...
  it('404s an allowlisted name whose built file is missing', async () => {
    ...
    try {
      ...
      await handler(req('GET', '/sidebar/bundle/terminal.js'), res as unknown as SidebarHttpResponse)
  ...
  it('gates non-GET/HEAD methods with 405', async () => {
    ...
    try {
      const res = fakeRes()
      await handler(req('POST', '/sidebar/bundle/editor.js'), res as unknown as SidebarHttpResponse)
