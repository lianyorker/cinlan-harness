...
/** One active child plus one persisted resumable child. */
function mixedSnapshot(): SidebarSessionList {
  return {
    current: 'root',
    byId: {
      root: { id: 'root', displayTitle: '主会话' },
      a: { id: 'a', displayTitle: 'A', origin: 'subagent', parentId: 'root', running: true },
      b: { id: 'b', displayTitle: 'B', origin: 'subagent', parentId: 'root', running: false },
    },
    subagentsByParent: {
      root: {
        entries: [
          { kind: 'child', id: 'a', activity: 'running', hasChildren: false, mode: 'one-shot', label: 'A' },
          { kind: 'child', id: 'b', activity: 'inactive', hasChildren: false, mode: 'continuable', label: 'B' },
        ],
        parentAvailable: true,
        state: 'ready',
        error: null,
...
describe('SubagentView live polling', () => {
  it('separates running children from resumable history in labels and totals', () => {
    ...
    const { container, unmount } = mount(
      createElement(SubagentView, { sessionId: 'root', active: false, ctx: makeCtx(store, historySpy) }),
    )

    expect(container.textContent).toContain('1 运行中 · 1 历史')
    expect(container.textContent).toContain('B · 已结束 · 可恢复')
    expect(container.textContent).not.toContain('空闲')
    unmount()
  })
