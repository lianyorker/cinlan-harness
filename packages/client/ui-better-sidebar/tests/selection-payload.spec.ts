...
describe('headerOf', () => {
  it('projects the path relative to the session cwd', () => {
    expect(headerOf(['/p', 'docs', 'plan.md'].join('/'), '/p', { start: 12, end: 12 })).toBe(['docs', 'plan.md'].join('/') + ':12')
  })

  it('falls back to the absolute path when the cwd is unknown', () => {
    expect(headerOf(['/p', 'docs', 'plan.md'].join('/'), undefined, { start: 3, end: 3 })).toBe(['/p', 'docs', 'plan.md'].join('/') + ':3')
  })
