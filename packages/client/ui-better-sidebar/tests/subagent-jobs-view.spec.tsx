...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (method === 'jobs.output') {
      ...
      // bash-9 stands for a running job that has not produced output yet.
      return jsonResponse({
        ok: true,
        value: {
...
describe('SubagentView background jobs', () => {
  ...
  it('shows an empty state before the job produces output', async () => {
    ...
    await act(async () => { row.click() })
    expect(container.textContent).toContain('暂无输出')
    expect(container.textContent).not.toContain('job_output')
    unmount()
  })
