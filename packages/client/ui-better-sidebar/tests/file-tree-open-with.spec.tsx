...
async function mountTree(overrides: {
  ...
  await act(async () => {
    ...
    props.openWithTargets = has('openWithTargets') ? overrides.openWithTargets : targets
    props.openWithPinned = has('openWithPinned') ? overrides.openWithPinned : ['vscode']
    props.openWithSsh = has('openWithSsh') ? overrides.openWithSsh : false
    root.render(createElement(FileTree, props as unknown as ComponentProps<typeof FileTree>))
  })
  return {
    container,
    onOpenWith,
