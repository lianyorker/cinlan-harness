...
import { launchExternal, revealCommand, urlCommand, validateBrowserUrl, validateExternalUrl } from '../src/open-external.ts'
...
describe('launchExternal validation (pre-spawn)', () => {
  ...
  it('rejects a non-http(s) target for the browser action', () => {
    expect(() => launchExternal('browser', 'vscode://file/x')).toThrow(SidebarError)
    expect(() => launchExternal('browser', 'not a url')).toThrow(SidebarError)
  })
})

describe('validateBrowserUrl', () => {
  it('accepts http(s) URLs (transcript links to real pages)', () => {
    expect(validateBrowserUrl('https://example.com/page?a=1')).toBe('https://example.com/page?a=1')
    expect(validateBrowserUrl('http://192.168.1.1/')).toBe('http://192.168.1.1/')
  })

  it('rejects custom schemes and unparsable strings (the inverse of validateExternalUrl)', () => {
    expect(() => validateBrowserUrl('vscode://file/x')).toThrow(SidebarError)
    expect(() => validateBrowserUrl('mailto:a@b.c')).toThrow(SidebarError)
    expect(() => validateBrowserUrl('not a url')).toThrow(SidebarError)
  })
})
