...
import { SIDE_THREAD_PREFIX } from '../sidechat-core.ts'
import {
  collectTreeJobs,
  formatJobDuration,
  isJobLive,
...
/** The secondary line of one card: title · activity · mode (skips empty parts). */
...
function CatalogRows({
  ...
  const visibleEntries = (catalog?.entries ?? []).filter((entry) => {
    if (entry.kind === 'child') return !(entry.label?.startsWith(SIDE_THREAD_PREFIX) ?? false)
    return !(byId[entry.id]?.displayTitle.startsWith(SIDE_THREAD_PREFIX) ?? false)
  })
  return (
    <>
      {emptyLoading && (
 ...
 * sidebar body (sticky, terminal-like) shows the SELECTED job's retained
 * output, refreshed every {@link JOB_POLL_MS} while the job runs and the
 * page is visible. The registry gives the browser an independent bounded
 * reader, so opening the pane never advances the model's `job_output`
 * cursor. A single dock — not a panel per row — keeps the
 ...
 * `session/jobs` push mirror. Clicking a row feeds independently retained
 * output to the shared bottom dock without advancing the model's cursor; live
...
export function SubagentView(props: {
  ...
  const rootCatalog = rootId === undefined
    ? undefined
    : catalogs[rootId as keyof typeof catalogs]
  const rootSummary = rootId === undefined
    ? undefined
    : byId[rootId as keyof typeof byId]
  ...
  const countLabel = totals.count === 0
    ? undefined
    : t('subagentCountStatus', {
      running: totals.runningCount,
      history: totals.count - totals.runningCount,
    })
