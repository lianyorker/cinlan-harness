...
export function apply(ctx: Context): void {
  ...
  const preferences = new SidebarPreferencesController(
    ctx.settingsScope.bind<SidebarPrefs>({
      namespace: SIDEBAR_PREFS_NS,
      decode: value => parsePrefs(value),
    }),
    sidebarStore,
  )
  ctx.effect(() => () => { preferences.dispose() }, 'dsh-better-sidebar: preferences scope')
      ...
      () => {
        try {
          ...
          return registerLinkInterception({
            ...
            // a modified-click or right-click on a transcript link hands the
            // http(s) URL to the OS browser through the host opener. A plain
            // browser leaves this unset and keeps its native new-tab gesture.
            openExternalBrowser: parseDesktopEnv().desktop
              ? (external) => { void api.openExternal({ action: 'browser', url: external }) }
              : undefined,
          })
        } catch (error) {
          fail('interception', error)
