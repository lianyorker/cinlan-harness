...
/** Whether a click asks to open the link OUTSIDE the GUI: a modified left
 *  click (Ctrl/Cmd/Shift/Alt) — the browser's "new tab/window" gesture. In a
 *  desktop shell the locked webview cannot honor that navigation, so the host
 *  opener takes over; in a plain browser the default gesture already works and
 *  this path is not wired. Right-click is handled separately (contextmenu). */
export function isExternalOpenClick(event: { button: number; metaKey: boolean; ctrlKey: boolean; shiftKey: boolean; altKey: boolean }): boolean {
  return event.button === 0 && (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey)
}
...
export function registerLinkInterception(opts: {
  ...
  selfOrigin: string
  /** Open an external http(s) link in the OS browser through the host. Wired
   *  ONLY in a desktop shell, whose webview cannot navigate to an external
   *  page: a modified left click or a right click on an external link routes
   *  here instead of a dead in-webview navigation. Absent in a plain browser,
   *  where the native gestures already open a real tab. */
  openExternalBrowser?: (url: string) => void
}): () => void {
  const externalAnchorUrl = (event: MouseEvent): string | null => {
    ...
    return shouldInterceptLink(anchor.href, opts.selfOrigin)
  }
  const onClick = (event: MouseEvent): void => {
    if (event.defaultPrevented) return
    // Desktop: a modified left click (the "new tab" gesture) on an external
    // link cannot open in the locked webview, so hand it to the OS browser.
    if (opts.openExternalBrowser !== undefined && isExternalOpenClick(event)) {
      const external = externalAnchorUrl(event)
      if (external === null) return
      event.preventDefault()
      opts.openExternalBrowser(external)
      return
    }
    if (!isPlainLeftClick(event)) return
    const url = externalAnchorUrl(event)
    if (url === null) return
  ...
  const onContextMenu = (event: MouseEvent): void => {
    // Desktop only: right-clicking an external link opens it in the OS
    // browser (the webview's native context menu cannot reach a real tab).
    if (opts.openExternalBrowser === undefined) return
    if (event.defaultPrevented) return
    const external = externalAnchorUrl(event)
    if (external === null) return
    event.preventDefault()
