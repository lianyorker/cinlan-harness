...
function threadDisplayTitle(title: string): string {
  if (title === SIDE_NEW_THREAD_MARKER) return t('sideChatUntitled')
  return title.startsWith(SIDE_THREAD_PREFIX) ? title.slice(SIDE_THREAD_PREFIX.length) : title
}
    ...
    case 'user':
      return (
        <div key={`${row.kind}:${row.seq}`} className={css.sidechatUser}>
          <MarkdownText text={row.text} labels={{ code: labels, footnotes: '' }} />
        </div>
      )
    case 'assistant':
      return (
...
export function SideChatView(props: {
  ...
  const summary = threadId === undefined
    ? undefined
    : list.byId[threadId as keyof typeof list.byId]
