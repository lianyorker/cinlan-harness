...
const TabContent = memo(function TabContent(props: TabContentProps) {
  ...
  return createElement(
    ...
    createElement(descriptor.component, {
      ctx, store, preferences, scope, tab, visible, expanded,
...
export function Sidebar(props: { ctx: Context; store: SidebarStore; preferences?: SidebarPreferencesController }) {
  ...
  /**
   * Seed an empty bottom workbench with a terminal when it becomes visible.
   * Persisted blank panels recover on mount, and a failed attempt may retry on
   * the next expansion instead of leaving the panel permanently empty.
   */
  const bottomWasOpenRef = useRef<boolean | undefined>(undefined)
  useEffect(() => {
    if (narrow || state === undefined) return
    const wasOpen = bottomWasOpenRef.current
    bottomWasOpenRef.current = state.bottomOpen
    if (wasOpen === true || !state.bottomOpen) return
    if (allLeaves(state.bottomSplits).some(leaf => leaf.tabs.length > 0)) return
    ...
    <div data-dsh-panel-host {...osFileDragShield}>
      ...
      */}
      <div
        ...
        data-panel-open={state.panelOpen || undefined}
      >
        {/*
