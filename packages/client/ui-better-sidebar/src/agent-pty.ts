...
/** Read the mutable exit flag without retaining the previous flow narrowing. */
function hasExited(handle: AgentTerminalHandle): boolean {
...
export class AgentPtyRegistry {
  ...
  signal(uuid: string, signal: AgentTerminalSignal): void {
    ...
    // Windows queues deferred callbacks in node-pty, so a named signal throws
    // outside this call stack; the default kill is its TerminateProcess path.
