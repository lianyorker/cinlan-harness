...
export const SIDE_THREAD_PREFIX = 'Side: '
...
export const SIDE_NEW_THREAD_MARKER = 'Side: New thread'
...
export function sideThreadRows(
  ...
  for (const summary of Object.values(byId)) {
    ...
    if (!summary.displayTitle.startsWith(SIDE_THREAD_PREFIX)) continue
...
export function sideLabel(question: string): string {
  ...
  const max = Math.max(1, LABEL_MAX_CHARS - SIDE_THREAD_PREFIX.length)
  const body = flat.length > max ? `${flat.slice(0, max - 1)}…` : flat
  return `${SIDE_THREAD_PREFIX}${body}`
}
