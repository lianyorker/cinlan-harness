  ...
  return {
    'sidechat.start': async (payload: unknown) => {
      ...
      const inheritance = buildSidechatInheritance(
        // @ts-expect-error Phase 4: Session.events API drift from staged-final reference
      ...
      const label = question === '' ? SIDE_NEW_THREAD_MARKER : sideLabel(question)
      ...
      const options: CreateAgentOptions = {
        ...
        meta: {
          ...
          // @ts-expect-error Phase 4: meta.seedLength API drift from staged-final reference
      ...
      if (question === '') {
        ...
        pinTitle(SIDE_NEW_THREAD_MARKER)
      } else {
