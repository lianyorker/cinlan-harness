...
export interface SidebarJobsService {
  /** Observe retained output without consuming the model-facing cursor. */
  peekOutput(id: string, caller?: SidebarAgent): { text: string; truncated: boolean }
...
/** Standard Client settings binder consumed by feature-owned preference UI. */
export interface SidebarSettingsScopeBinder {
  /** Bind one registered namespace to the calling plugin's lifecycle. */
...
/**
 * Local Context type for this plugin's source: intersects the real cordis
 * Context with structural mirror types for the services this plugin consumes.
 * Using a local type (instead of `declare module`) avoids polluting other
 * packages' Context type in the monorepo's shared compilation.
 */
...
/**
 * Minimal global augmentation: only adds `betterSidebar` to the cordis
 * Context so other packages can access it via `ctx.betterSidebar`. Service
 * members (webServer, sessions, settings, etc.) are NOT re-declared here —
 * their real augmentations from the owning packages stay authoritative.
 */
