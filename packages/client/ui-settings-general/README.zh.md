...
使用本包可为 dsh Web 客户端提供全页面 Settings 界面、由功能包贡献的导航，以及依次进行的首次运行引导。用户可以从侧边栏打开设置页面、按标签搜索分区，并在宿主为回环浏览器提供本地配置文件时访问该文件。各功能包提供自己的设置行、分区和引导步骤；本包提供共享的界面展示，但不添加引导文案或「通用」分区的内置行。
...
用户通过侧边栏底部的 Settings 控件进入外壳；功能插件通过本外壳所投影的 slot 账本贡献自己的页面与引导步骤。打开 Settings 后，视口被全页面区域替换：左侧导航栏包含返回按钮和搜索输入框（实时过滤分区标签），右侧为独立滚动的内区。每个分区可通过 `settings.section.icon` keyed slot 注册导航图标。外壳渲染页面、由 `settings.section` 条目构建的导航，以及每次只挂载一个的引导步骤。
