...
使用本包可为 dsh Web 客户端提供全页面 Settings 界面、由功能包贡献的导航，以及依次进行的首次运行引导。用户可以从侧边栏打开设置页面、搜索页面和已登记的具体设置，并在宿主为回环浏览器提供本地配置文件时访问该文件。各功能包提供自己的设置行、分区和引导步骤；本包提供共享的界面展示，但不添加引导文案或「通用」分区的内置行。
...
用户通过侧边栏底部的 Settings 控件进入外壳；功能插件通过本外壳所投影的 slot 账本贡献自己的页面与引导步骤。打开 Settings 后，视口被全页面区域替换：左侧分组导航包含返回按钮和搜索输入框，右侧内容独立滚动。六个有序分组组织已挂载页面，空分组不显示。页面通过 portal 渲染在处于 inert 状态的应用根节点之外，关闭后恢复根节点原状态和入口焦点。搜索保留已挂载分区但将其隐藏，以保留未保存草稿，并展示匹配的页面标签或功能所有者提供的字段标题、说明与关键词。选择字段后打开所属分区及可选标签页，再滚动并聚焦控件。Ctrl/Command+K 聚焦搜索；Escape 先清除搜索，再关闭设置。窄屏通过原生模态抽屉管理导航焦点，关闭后将焦点还给菜单按钮。每个分区可通过 `settings.section.icon` keyed slot 注册导航图标。外壳渲染页面、由 `settings.section` 条目构建的导航，以及每次只挂载一个的引导步骤。
...
导航将已挂载的 `settings.section` 条目与[设置域元数据](../ui-settings/README.zh.md) 关联。slot 条目拥有页面标签、顺序和图标；元数据拥有分组归属及本地化字段定位目标。未声明分组的页面归入扩展管理。稳定的行快照在 slot、元数据和语言变化后刷新。没有已挂载页面的元数据不会创建导航入口。外壳提供通用、模型和插件页标题，其他分区保留其功能所有者的标题。内容区域在当前页面旁渲染键匹配的 `settings.section.extension`，共享该页面的可见性与搜索目标范围。引导账本按升序投影；当前注册方会收到该条目的 id、`complete()` 与 `openSection(id)` 回调，完成或跳过当前步骤后，所有权转交给下一项。
