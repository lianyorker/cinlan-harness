...
导航将已挂载的 `settings.section` 条目与[设置域元数据](../ui-settings/README.zh.md) 关联。slot 条目拥有页面标签、顺序和图标；元数据拥有分组归属及本地化字段定位目标。未声明分组的页面归入扩展管理。稳定的行快照在 slot、元数据和语言变化后刷新。没有已挂载页面的元数据不会创建导航入口。外壳提供通用、模型和插件页标题，其他分区保留其功能所有者的标题。引导账本按升序投影；当前注册方会收到该条目的 id、`complete()` 与 `openSection(id)` 回调，完成或跳过当前步骤后，所有权转交给下一项。
