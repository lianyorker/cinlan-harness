...
使用本包可以为新的 Web GUI 会话选择 agent preset、在会话标题中查看当前 preset，并在设置中管理可用 preset。preset 在会话创建时即固定，因此更改选择或默认值只影响此后创建的会话。如果部署未提供任何 preset，会话选择器保持隐藏，设置页说明会话使用宿主组装。
...
设置搜索只索引默认值、内置与自定义分区的本地化标签及说明，不包含预设元数据、组装文本或显示路径。搜索条目随页面注册一起移除；空自定义列表保留定位锚点和创建说明。页面沿用设置外壳的内容宽度与响应式原生行布局。
