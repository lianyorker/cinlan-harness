...
使用本包可以为新的 Web GUI 会话选择 agent preset、在会话标题中查看当前 preset，并在设置中管理可用 preset。已开始的会话保留其 preset；设置变更也可更新当前空白会话。如果部署未提供任何 preset，会话选择器保持隐藏，设置页说明会话使用宿主组装。
...
与设置与对话包一起挂载本插件；preset 表面随即出现在各自槽位渲染之处。启用模式选择时，新建会话 chip 以 Host 生效的默认值打开并暂存一个选择，落到下一个空白会话上；暂存一经使用即被清空，因此再下一个新会话重新以默认值打开。

### 管理名单

模式选择开关仅写入 `agent-presets.modeSelectionEnabled`。关闭后，新建会话选择器隐藏，待应用的选择和拒绝横幅被清除，并使用部署默认值，而不覆盖已保存的默认值。重新开启后恢复已保存的默认值。设置页回读生效的默认值，仅当保存前捕获的空白会话仍为当前空白会话时同步它；已开始的会话及其标题标签保持不变。模式选择关闭时，默认值选择与创造模式被禁用；查看、复制、打开和删除预设仍然可用。

设置页提供用于未来会话的默认值选择器，并分别列出内置与自定义资源。内置预设在只读查看器中打开；复制只接收现有预设来源、标识符和可选显示名称。自定义预设保留打开目录或显示路径操作，以及确认后删除操作。损坏的预设不能设为默认或复制，但自定义文件仍可访问以便修复或删除。浏览器不接收任意组装文本。Host 失败保持可见，复制失败保留输入草稿。

`system` 信任级别的 `security-research` 行使用本地化的安全研究名称与描述。本插件不会向名单添加该预设；可选组合包负责它是否可用。用户创作的元数据保持不翻译。

设置搜索只索引模式选择、默认值、内置与自定义分区的本地化标签及说明，不包含预设元数据、组装文本或显示路径。搜索条目随页面注册一起移除；空自定义列表保留定位锚点和创建说明。页面沿用设置外壳的内容宽度与响应式原生行布局。

### 对话式入口

模式选择已启用且名单携带自指的 `cordis` preset 时，一张虚线添加卡会暂存它并开启新会话——分区关闭设置面板，新建会话 chip 自己的应用器负责组装工作区流程产出的空白会话。
...
- **组装编辑对页面不可见**——文件在浏览器之外编辑，线上不广播文件变动，因此名单只在自身操作、`settings/document-updated` 与 `connection/reset` 时重读，而非每次磁盘编辑。
