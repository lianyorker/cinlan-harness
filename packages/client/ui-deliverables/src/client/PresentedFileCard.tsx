...
export function PresentedFileCard({ file, cwd, phase, host, onPreview, onAction, t }: {
  ...
  const preview = async () => {
    ...
    try { await onPreview() }
    catch { setPreviewError(true) } // The sidebar rejects unavailable Session directories or files.
  }
  ...
  const status = previewError ? t('presented.previewError') : phase === undefined
  ...
  return <div className={css.file} data-presented-file>
    <button type="button" className={css.cardPreview} title={resolveWorkspacePath(cwd, file.path)}
      aria-label={t('presented.previewCard', { name: file.path })} onClick={() => { void preview() }} />
    ...
    <div className={css.fileBody}>
      <div className={css.details}>
        ...
        <span className={css.description} role={phase === undefined && !previewError ? undefined : 'status'}
          data-error={previewError || phase === 'error' || phase === 'revealError' || phase === 'nativeUnavailable' ? true : undefined}>
      ...
      <div className={css.split}>
        <button ref={previewRef} type="button" className={css.open}
          ...
          onClick={() => { void preview() }}>{t('presented.action')}</button>
        <Menu className={css.menuAnchor} open={menuOpen && !menuDisabled} portal align="end" onClose={() => { setMenuOpen(false); menuRef.current?.focus() }}
