...
export interface DeliverablesInjected {
  ...
  /** Preview a source through the existing sidebar and propagate failures to its card. */
  previewPresented: (
    sessionId: SessionId, cwd: string | undefined, path: string, fallback: (path: string) => void | Promise<void>,
  ) => void | Promise<void>
 ...
 */
export function DeliverablesTail(props: PropsRuntime<'conversation.chat.turnCards'> & PropsLocale<typeof NS> & InjectFace<DeliverablesInjected>) {
...
export function Deliverables({
  ...
  useChangesSummary, reloadPresentedHost, loadChangesSummary, previewPresented,
  ...
  }, [host, reloadPresentedHost, matched.presented.length])
  return <>
    ...
    >
      ...
      <div className={css.presented} data-presented-files-row data-single={matched.presented.length === 1 ? true : undefined}>
        {presented.map(file => <PresentedFileCard key={`${file.seq}:${file.index}`} file={file} cwd={cwd}
          ...
          onPreview={() => previewPresented(sessionId, cwd, file.path, openFile)}
