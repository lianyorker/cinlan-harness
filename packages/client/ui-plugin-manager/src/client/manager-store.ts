...
export type FailedAction = 'enable' | 'disable' | 'uninstall' | 'rowEnable' | 'rowDisable' | 'openPlugins'
...
export type ManagerNotice =
  ...
  | { readonly kind: 'desktop-packages'; readonly seq: number }
  | {
...
export interface PackageView {
  ...
  /** Whether the Host permits removing this dependency. */
...
export interface PluginManagerState {
  ...
  /** Desktop package actions open its native window; row configuration still uses the Host. */
  readonly packageManagement: 'host' | 'desktop'
...
export interface PluginManagerFace {
  ...
  /** Read the Host again. */
  refresh: () => void
  /** Open Host installation or the native Desktop Plugins window; absent bridges show menu guidance. */
  openInstall: () => void
...
export class PluginManagerController {
   ...
   * @param desktop - native app marker with an optional Plugins window opener; package operations stay in the shell.
   * @param reloadDesktop - refresh the renderer after a changed, applied Desktop row and settled inventory; the Host stays alive.
   */
  constructor(
    private readonly ctx: ClientContext,
    private readonly desktop?: { readonly openPlugins?: () => Promise<void> },
    private readonly reloadDesktop?: () => void,
  ) {
    this.store = createSnapshotStore<PluginManagerState>({
      status: 'idle', packages: [], busy: [], notice: null, packageManagement: desktop === undefined ? 'host' : 'desktop',
  ...
  inject(configLedger: HostObservable<ConfigLedger>): PluginManagerFace {
    return {
      ...
      openInstall: () => { void this.openInstall() },
      closeInstall: () => {
      ...
      setEnabled: (packageName, enabled) => {
        if (this.desktopPackages()) return
        void this.run(packageName, { packageName, action: enabled ? 'enable' : 'disable' }, async () => {
          this.applied(await this.manager().setBundleEnabled(packageName, enabled), packageName)
        })
      },
      uninstall: (packageName) => {
        if (this.desktopPackages()) return
        this.pendingConfirm = () => this.run(packageName, { packageName, action: 'uninstall' }, async () => {
          this.applied(await this.manager().removeBundle(packageName), packageName)
        })
      ...
      setRowEnabled: (entryId, enabled) => { void this.setRowEnabled(entryId, enabled) },
  ...
  private async read(): Promise<void> {
    try {
      do {
        ...
        const inventoryRemote = this.ctx.get('remote.pluginInventory') as ClientContext['remote']['pluginInventory'] | undefined
        if (inventoryRemote === undefined || this.ctx.get('remote.pluginManager') === undefined) {
          this.patch({ status: 'unavailable', packages: [] })
          continue
        }
        const inventory = await answer(() => inventoryRemote.list())
        if (generation !== this.generation) return
        if (!inventory.ok) {
          this.patch({ status: 'error' })
          continue
        }
        this.patch({ packageManagement: this.desktop !== undefined || inventory.value.packageManagement === 'desktop' ? 'desktop' : 'host' })
        if (inventory.value.managementAvailable !== true) {
          this.patch({ status: 'unavailable', packages: [] })
          continue
        }
        const [bundles, plugins] = await Promise.all([
          answer(() => this.manager().listBundles()),
          answer(() => this.manager().listPlugins()),
        ])
        if (generation !== this.generation) return
  ...
  private async setRowEnabled(entryId: PluginEntryId, enabled: boolean): Promise<void> {
    let reload = false
    await this.run(rowKey(entryId), { packageName: entryId, action: enabled ? 'rowEnable' : 'rowDisable' }, async () => {
      const result = await this.manager().setPluginEnabled(entryId, enabled)
      this.applied(result, entryId)
      reload = this.desktopPackages() && result.ok && result.value.changed && result.value.application === 'applied'
    })
    if (reload && !this.disposed && this.getSnapshot().status === 'ready') this.reloadDesktop?.()
  }

  private desktopPackages(): boolean {
    return this.getSnapshot().packageManagement === 'desktop'
  }

  private async openInstall(): Promise<void> {
    if (this.disposed) return
    if (this.desktopPackages()) {
      if (this.desktop?.openPlugins === undefined) {
        this.patch({ notice: { kind: 'desktop-packages', seq: ++this.noticeSeq } })
        return
      }
      try {
        await this.desktop.openPlugins()
      } catch (error) {
        this.patch({ notice: failedNotice(error, { action: 'openPlugins' }, ++this.noticeSeq) })
      }
      return
    }
    if (!isInstallPending(this.getSnapshot().install.phase)) this.patch({ install: { ...IDLE_INSTALL, open: true }, highlight: null })
  }
  ...
  private async runInstall(): Promise<void> {
    ...
    const inspected = await answer(() => this.manager().inspect(spec, controller.signal))
  ...
  private async startInstall(subject: InstallSubject, approvedBuilds?: readonly string[]): Promise<void> {
    ...
    const result = await answer(() => this.manager().installBundle(spec, {
    ...
    } else if (result.value.bundle === undefined) {
      this.patchInstall({ phase: 'failed', runs: settledRuns(runs, null), failure: { code: 'ambiguous-install', reason: '' } })
    } else {
      this.patchInstall({
        phase: 'done',
  ...
  private async cancelInstall(closeAfter = false): Promise<void> {
    ...
    const result = await answer(() => this.manager().cancelInstall(requestId))
  ...
  private async enableInstalled(): Promise<void> {
    ...
    if (name !== null) {
      const result = await answer(() => this.manager().setBundleEnabled(name, true))
      if (this.disposed) return
      try {
  ...
  private manager(): ClientContext['remote']['pluginManager'] {
    const remote = this.ctx.get('remote.pluginManager') as ClientContext['remote']['pluginManager'] | undefined
    if (remote === undefined) throw new Error('Plugin management is unavailable')
    return remote
  }
...
/** Transport rejections retain their diagnostics and settle the same UI as Remote failures. */
async function answer<T>(call: () => Promise<Answer<T>>): Promise<Answer<T>> {
  try {
    return await call()
  } catch (error) {
    return { ok: false, error: { code: 'transport', message: reasonOf(error) } }
  }
}
