...
function bench(overrides: Partial<Record<string, ReturnType<typeof vi.fn>>> = {}, reloadDesktop?: () => void) {
  ...
  const ctx = { get: (name: string) => name === 'remote.pluginManager' ? plugins : name === 'remote.pluginInventory' ? inventory : undefined } as never
  const controller = new PluginManagerController(ctx, undefined, reloadDesktop)
...
describe('PluginManagerController', () => {
  it('refreshes the Desktop renderer once after a changed applied row and settled inventory', async () => {
    const reload = vi.fn()
    const b = bench({ inventory: vi.fn().mockResolvedValue(ok({ entries: [], managementAvailable: true, packageManagement: 'desktop' })) }, reload)
    await b.controller.load()
    const refreshed = deferred<ReturnType<typeof ok<BundleInfo[]>>>()
    b.plugins.listBundles.mockReturnValueOnce(refreshed.promise)
    b.face.setRowEnabled(ROW_ENTRY, false)
    b.face.setRowEnabled(ROW_ENTRY, false)
    await vi.waitFor(() => { expect(b.plugins.listBundles).toHaveBeenCalledTimes(2) })
    expect(b.plugins.setPluginEnabled).toHaveBeenCalledExactlyOnceWith(ROW_ENTRY, false)
    expect(reload).not.toHaveBeenCalled()
    refreshed.resolve(ok([BUNDLE]))
    await vi.waitFor(() => { expect(reload).toHaveBeenCalledExactlyOnceWith() })
    b.controller.dispose()
  })

  it.each([
    { name: 'unchanged result', result: ok({ ...APPLIED, changed: false }) },
    { name: 'restart requirement', result: ok({ ...APPLIED, application: 'restart-required' as const }) },
    { name: 'higher-priority override', result: ok({ ...APPLIED, application: 'overridden' as const }) },
    { name: 'application failure', result: ok(failed({ code: 'operation-error', diagnostic: 'write failed' })) },
    { name: 'Host refusal', result: refused('UNKNOWN', 'refused') },
  ])('keeps the Desktop renderer for $name', async ({ result }) => {
    const reload = vi.fn()
    const b = bench({
      inventory: vi.fn().mockResolvedValue(ok({ entries: [], managementAvailable: true, packageManagement: 'desktop' })),
      setPluginEnabled: vi.fn().mockResolvedValue(result),
    }, reload)
    await b.controller.load()
    b.face.setRowEnabled(ROW_ENTRY, false)
    await vi.waitFor(() => { expect(b.plugins.listBundles).toHaveBeenCalledTimes(2) })
  ...
  it('keeps the Desktop renderer when refreshed inventory fails or the controller is disposed', async () => {
    const reload = vi.fn()
    const b = bench({ inventory: vi.fn().mockResolvedValue(ok({ entries: [], managementAvailable: true, packageManagement: 'desktop' })) }, reload)
    await b.controller.load()
    b.inventory.list.mockResolvedValueOnce(refused('UNKNOWN', 'connection lost'))
    b.face.setRowEnabled(ROW_ENTRY, false)
    ...
    await vi.waitFor(() => { expect(b.plugins.listBundles).toHaveBeenCalledTimes(2) })
    const settled = b.controller.load()
    b.controller.dispose()
    refreshed.resolve(ok([BUNDLE]))
    await settled
    expect(reload).not.toHaveBeenCalled()
  ...
  it('keeps the Web renderer after applied row and bundle changes', async () => {
    const reload = vi.fn()
    const b = bench({}, reload)
    await b.controller.load()
    b.face.setRowEnabled(ROW_ENTRY, false)
    await vi.waitFor(() => { expect(b.plugins.listBundles).toHaveBeenCalledTimes(2) })
    b.face.setEnabled(BUNDLE.name, true)
    await vi.waitFor(() => { expect(b.plugins.listBundles).toHaveBeenCalledTimes(3) })
    await b.controller.load()
    expect(reload).not.toHaveBeenCalled()
    b.controller.dispose()
  })
  ...
  it('enables what a finished install added from its screen, closes, and marks it in the list', async () => {
    ...
    // Missing bundle identity cannot become an installed confirmation.
    face.openInstall()
    face.editInstallSpec('lib')
    face.runInstall()
    await vi.waitFor(() => { expect(state().install.phase).toBe('failed') })
    expect(state().install.failure).toMatchObject({ code: 'ambiguous-install' })
