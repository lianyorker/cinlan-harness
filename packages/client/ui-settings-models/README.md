...
Use the Models settings page to configure API keys, choose DeepSeek's request protocol, edit model catalogs and input capabilities, and add custom pi-ai providers. Provider cards preserve existing model settings and show configuration errors beside the affected provider. API keys are stored write-only under credential references. First-run users receive a versioned internal-testing notice followed, when needed, by the official DeepSeek credential dialog.
...
Open the Models page from the Settings navigation to see every configured provider as a row. A whole-section provider whose key is not configured anywhere renders as its open setup card instead, but only in the first-run posture and only until the user closes that card. Each card kind owns its own open state, so closing one never discards a draft in another. The Models entry belongs to the AI and models settings group. Settings search finds the provider list and both add controls through localized labels and public field names; credentials, current values, and user-defined provider names are excluded.
...
The collapsed **Customized settings** fold contains `baseURL` and the model catalog for both families, DeepSeek's **API protocol**, and the **display name** and **API protocol** of a custom pi-ai route. DeepSeek defaults to `chat-completions`; choose `messages` explicitly for a Messages endpoint. Changing the protocol preserves the configured Base URL and model list. The DeepSeek Base URL placeholder remains the public endpoint, and custom gateways remain editable. Provider configuration errors appear on the affected card without blocking its editor.

Profile `headers` remain deployment configuration in `settings.yaml` or Cordis config and have no Models-page editor. The Provider ID stays fixed because it identifies the profile, request route, and credential reference. Reasoning effort remains a per-model selection outside this provider card. Each model row edits `id`, optional display `name`, and optional `contextWindow`/`maxTokens`; existing fields outside the curated editor survive edits.

### Model input capabilities

Expand a model's **Capacities** control to choose **Text**, **Image**, or **Text and image**. DeepSeek stores this choice as `inputModalities`; pi-ai stores it as `input`. Pi-ai rows without an explicit nonempty input list display the installed model's discovered capabilities, then the provider's `defaultInput`, then text. Reading that catalog does not change the draft. A failed lookup shows its diagnostic and leaves manual editing available. Changing a DeepSeek row to text-only removes its `imagePixelBudget` and `imageMaxBytes`, which are invalid without image input. Other model fields and rows remain intact.

### Adding and deleting providers

The add flow is a card carrying the dormant-directory provider select — a bare-mounted `llm-pi-ai` offers its whole installed catalog before any route exists. **Add a custom provider** declares a route pi-ai does not ship; the create card asks for a unique **Provider ID**, an endpoint, a protocol, and at least one uniquely-identified model, because nothing can default those. The Base URL is trimmed before create or discovery; malformed and non-HTTP(S) URLs show a field error and block both actions. **Fetch available models** asks the `llm/discoverModels` Remote about the endpoint the form shows, so adding a provider is one pass instead of save-then-return; the reply opens a searchable picker. **Add selected** adds candidates to the draft; **Apply** or **Create provider** persists the draft. Each selected candidate copies its id, display name, context window, output-token cap, and input capabilities into the editable row when disclosed, while an existing row retains its user-tuned values. Search matches model ids and optional display names without clearing hidden selections. **Select all** adds the visible results, while **Deselect all** clears the entire selection so hidden results cannot be adopted accidentally. A row is deletable only when the user layer alone carries it (removal restores the composition base), and its confirmation dialog names the provider.
...
The page never holds a full settings section: it holds only the REDACTED descriptor, so every edit lands as `settings.mutate` path ops against the stored section — a set per changed field, an unset per cleared one, and a single unset for a deleted provider row. Group and search metadata share the section slot declaration lifetime, including removal and HMR recovery. The existing add controls stay mounted while an editor is open so a search destination does not discard its draft.
