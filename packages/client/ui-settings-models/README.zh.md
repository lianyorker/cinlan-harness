...
使用 Models 设置页面配置 API 密钥、选择 DeepSeek 请求协议、编辑模型目录与输入能力，并添加自定义 pi-ai 提供方。提供方卡片保留已有模型设置，并在受影响的提供方旁显示配置错误。API 密钥以只写方式存入凭据引用。首次运行的用户会先看到版本化内测声明，再按需进入官方 DeepSeek 凭据弹窗。
...
收起的**自定义设置**折叠区包含两个家族的 `baseURL` 与模型目录、DeepSeek 的 **API 协议**，以及自定义 pi-ai 路由的**显示名称**与 **API 协议**。DeepSeek 默认使用 `chat-completions`；连接 Messages 端点时需显式选择 `messages`。切换协议会保留已配置的 API 地址与模型列表。DeepSeek API 地址的占位符仍显示公共端点，自定义网关仍可编辑。提供方配置错误显示在对应卡片上，编辑器仍可使用。

Profile `headers` 仍是 `settings.yaml` 或 Cordis 配置中的部署配置，Models 页面不提供编辑器。Provider ID 保持固定，因为它标识 profile、请求路由与凭据引用。推理等级仍在此提供方卡片之外按模型选择。每个模型行编辑 `id`、可选显示 `name` 与可选 `contextWindow`/`maxTokens`；精选编辑器之外的现有字段在编辑后仍会保留。

### 模型输入能力

展开模型的**容量**控件后，可以选择**文本**、**图像**或**文本与图像**。DeepSeek 将选项保存为 `inputModalities`，pi-ai 保存为 `input`。pi-ai 行没有显式非空输入列表时，会显示已安装模型发现的能力，其次使用提供方的 `defaultInput`，最后默认为文本。读取目录不会修改草稿。查询失败会显示诊断，同时仍可手动编辑。将 DeepSeek 行改为纯文本时，会移除没有图像输入便无效的 `imagePixelBudget` 与 `imageMaxBytes`。其他模型字段与模型行均保留。

### 新增与删除提供方

「新增」流程是一张承载休眠目录提供方选择框的卡片——裸挂载的 `llm-pi-ai` 在任何路由存在之前就能提供其完整的已安装 catalog。**添加自定义提供方**声明一条 pi-ai 不提供的路由；创建卡片会索要唯一的 **Provider ID**、端点、协议与至少一个可唯一识别的模型，因为没有东西能为它们兜底。创建或查询前会去除 API 地址两端的空白；格式无效或非 HTTP(S) 的地址会显示字段错误并阻止这两个操作。**获取可用模型**通过 `llm/discoverModels` Remote 查询表单显示的端点，因此新增提供方一次即可完成，而非先保存再返回；回复会打开可搜索选择器。**添加所选**将候选加入草稿，**保存**或**创建提供方**才会持久化草稿。每个选中候选会在提供方公布相应信息时，把 id、显示名、上下文窗口、最大输出 token 数与输入能力复制进可编辑行；已经存在的行保留用户调整过的值。搜索会匹配模型 id 与可选显示名称，且不会清除隐藏项的勾选状态。**全选**会加入可见结果，而**取消全选**会清空全部勾选，以免意外采用隐藏结果。只有用户层单独携带某行时，该行才可删除（删除会恢复组合基线），其确认对话框会指名该提供方。
