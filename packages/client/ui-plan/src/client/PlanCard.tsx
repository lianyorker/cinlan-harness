...
import { DocumentFileIcon, IconChevronRightOutline14 } from '@deepseek-ai/dsh-client-ui-primitives'
 ...
 */
export function PlanCards({ turn, useChat, openPlan, t }: PropsRuntime<'conversation.chat.turnCards'> & InjectFace<PlanOpenInjected> & PropsLocale<'plan'>) {
  ...
  return (
    <div className={css.cards} data-plan-artifacts>
      {plans.map(({ data: plan }) => <button key={plan.callId} type="button" className={css.card} data-plan-card={plan.callId}
        ...
        <span className={css.cardIcon}><DocumentFileIcon /></span>
