---
description: "Browser-host wire layer for the web GUI: Remote RPC, event-stream delivery with reconnect, Fetch routes, the /api HTTP bridge, and the browser-trust fence."
...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; shell-owned compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half always provides the carrier-neutral RPC and `GET`/`HEAD`/`POST` Fetch route registries. When a Web carrier is present it also owns the sole `/api` route, Fetch bridge, browser authentication, and Host/Origin checks; a shell-owned carrier dispatches the shared Fetch handler directly. Each route declares buffered or streaming request-body handling before the bridge reads any bytes. Typert Gateway claims generated Remote endpoints, feature packages register non-JSON responses such as Session-log downloads and raw file uploads, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state. A paired shell sets `ClientTransportHooks.authority: 'paired'`, `ownsHost: false`, and `pairingProtocolVersion: 1`; paired authority makes `ctx.connection.isLoopback` false even on localhost, while the server still owns Host authorization. Gateway adds the fixed `pairingVersion=1` query only for that explicit stream protocol. Browser raw-body transfer is provided by [`dsh-client-file-upload`](../file-upload/README.md). A disconnected browser aborts the request; the bridge cancels an unread late response body and stops writing to the closed response. Backpressure waits also settle when closure precedes listener installation.
...
Exact routes take precedence over prefixes; otherwise the longest matching prefix wins. A selected route's disallowed method returns 404 without trying another route or the RPC interceptor. Prefix contributions share exact-route body handling and caller-fiber withdrawal; the carrier and handler retain ownership of active request cancellation and settlement.

`createSharedFetchHandler('/api', access)` requires an explicit `HostConnectionAccess`. Each handle contains an in-process `identity` and revocation `signal`; delegated handles also authorize raw Fetch routes before their handlers read bodies. RPC handlers receive access as their fourth argument, and raw Fetch handlers receive it as their second. Gateway owns RPC, event, and Session policy keyed by that identity. The authenticated browser HTTP and WebSocket carriers reuse `connection.trustedAccess`, which is unique to each Connection instance and revoked when it is disposed. Other local carriers explicitly create and retain one `createTrustedConnectionAccess(signal)` handle for their lifetime. No missing-access path grants local trust.

Request cancellation and access revocation combine through `AbortSignal.any`. Revocation cancels pending upload and response-body readers, including an incomplete RPC envelope; body forwarding preserves backpressure. A raw-route authorization that completes after revocation cannot dispatch. The Host export `bridgeConnectionRequest(req, res, handler, maxRequestBodyBytes)` lets another authenticated carrier reuse the existing node:http bridge and buffered-body bound; that carrier owns its external authority, authentication, and delegated access before calling it.
