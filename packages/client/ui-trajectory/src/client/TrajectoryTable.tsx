...
function recordAttachments(
  ...
  return (blocks ?? []).flatMap((block, index) => {
    ...
    return [{
      ...
      metadata: [
        block.attachment?.mediaType ?? (/\.([^./\\]+)$/.exec(ref.name ?? '')?.[1] ?? '').toUpperCase(),
      ...
      {attachments.map(({ block, index, name, metadata }) => (
        <li className={css.attachmentRow} key={index}>
          {block.attachment !== undefined
            ...
            : <span className={css.attachmentIcon}><DocumentFileIcon /></span>}
...
function MarkdownRecordContent({
  ...
  const hasAttachments = record.cell.sourceBlocks?.some(block =>
    block.attachment !== undefined || block.file !== undefined) === true
