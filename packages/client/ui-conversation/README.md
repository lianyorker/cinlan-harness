...
The `conversation` registration permits explicitly scoped secondary occurrences through `slots.renderSessionView`. The optional `embedded` owner prop omits main navigation and width handles, and sizes the existing transcript and composer for the containing pane. Session actions, tool renderers, interaction takeovers, and input state remain the ordinary Conversation implementation.
...
The General preference row contributes localized search metadata with the stable `busy-send` anchor; the metadata shares the row’s slot registration lifetime.

[Keyboard](../keyboard/README.md) owns effective bindings for `conversation.submit` (Enter), `conversation.submitAccelerated` (Ctrl+Enter and Meta+Enter), and the popup actions `navigateUp`, `navigateDown`, `dismissPopup`, and `complete` under the same prefix (ArrowUp, ArrowDown, Escape, and Tab/Enter). Local Lexical dispatch reads current bindings and gives open completion menus priority before submission. Composition, key code 229, the Safari post-composition window, and submit repeats cannot send. Shift+Enter and unchanged editor history gestures remain intrinsic editing behavior. The empty-draft queue hint displays the effective accelerated shortcut. The placeholder appears only for an empty draft without attachments; typed whitespace hides it while Send remains disabled.
...
When a prompt fails with `session/writer-held`, the composer shows localized guidance to quit other running DSH instances and retry. Draft recovery follows the ordinary failed-submission rules.
