...
界面终端保留会话与标签身份。代理终端目标与关闭请求同时携带所属 Session 身份和现有注册表 UUID；提供方将属于其他 Session 的 UUID 视为不存在。连接 id 指向某一进程代次的一次连接；过期的输入、尺寸调整与关闭请求无法操作替代进程。额外的进程 id 在连接重建期间保持稳定。只读 `inspectUi` 查询已有界面进程，不创建进程或延长其生命周期；`closeUi` 在终止前比较该 id，因此持久化标签无需重新打开视图即可关闭。能力检查不创建进程。流依次发布初始信息、需要确认的输出帧和进程退出。确认输出表示 xterm 已完成消费该帧。提供方限制完整序列化帧与缓冲输出的大小，拥有原生暂停与恢复操作，并在连接结束时释放自身的暂停与监听器。

`listUi` 枚举此 Host 保留的存活 UI 终端，包括权威标题和进程身份。它不能创建进程或延长保留期限。`renameUi` 必须携带已观察到的进程身份，并在同一 PTY 上提交标题；重新连接通过 ready 帧返回标题。布局存储是 Client 投影，不会在 Host 重启后恢复原生进程。

`shells(sessionId)` 异步列出 Session 已捕获执行环境中安装的可执行文件，不创建进程。界面目标的可选 `shellPath` 为新进程选择其中一个可执行文件；省略时使用本地设置默认值或远程提供方默认值。已有进程在重连时保留自身的执行实例、目录和 Shell。代理终端目标不能选择 Shell。
...
本服务要求 Session 执行环境中的提供方。本地进程需要原生 PTY 依赖；SSH 进程需要有效的执行绑定和 POSIX 子进程终端提供方。
...
浏览器安全类型在生成的声明中保留仅类型的 Cordis 目标引用。这让核心与浮动终端的 `Context` 扩展在不同消费方导入顺序下仍合并到同一符号；编译器回归同时检查源码和声明消费方。
