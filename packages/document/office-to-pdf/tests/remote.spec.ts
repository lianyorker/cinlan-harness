...
const agent = { session: { id: SessionId('document-test') } } as Agent
...
it.each(['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'])('converts authorized %s bytes and returns the PDF under source identity', async (extension) => {
  ...
  const result = await ctx.officeToPdf.render(agent, path, 'foreground', new AbortController().signal)
  expect(read).toHaveBeenCalledExactlyOnceWith(target, expect.any(AbortSignal), 4)
  expect(authorize).toHaveBeenCalledExactlyOnceWith(agent, path, { offset: 0, length: 1 }, expect.any(AbortSignal))
...
it('preserves authorization failures without starting conversion', async () => {
  ...
  await expect(ctx.officeToPdf.render(agent, 'report.DOCX', 'foreground', new AbortController().signal)).rejects.toBe(failure)
...
it('keeps an unknown source size absent so the provider can reserve its input limit', async () => {
  ...
  await ctx.officeToPdf.render(agent, 'report.docx', 'foreground', new AbortController().signal)
...
it('refuses a source replaced while checking read access before cache lookup', async () => {
  ...
  await expect(ctx.officeToPdf.render(agent, 'report.docx', 'foreground', new AbortController().signal))
...
it('rejects unsupported extensions before reading a source', async () => {
  await expect(ctx.officeToPdf.render(agent, 'report.odt', 'foreground', new AbortController().signal))
...
it('cancels before reading and refuses a late authorized read after cancellation', async () => {
  await expect(ctx.officeToPdf.render(agent, 'report.docx', 'foreground', AbortSignal.abort())).rejects.toMatchObject({ code: 'gateway/cancelled' })
  expect(read).not.toHaveBeenCalled()
  ...
  const work = ctx.officeToPdf.render(agent, 'report.docx', 'foreground', controller.signal)
...
it('cancels and joins an outstanding conversion when the provider unloads', async () => {
  ...
  const work = ctx.officeToPdf.render(agent, 'report.docx', 'foreground', new AbortController().signal)
  const rejected = expect(work).rejects.toMatchObject({ code: 'gateway/cancelled' })
...
it('exposes the current renderer generation and refuses busy work before loading source bytes', async () => {
  ...
  await expect(ctx.officeToPdf.render(agent, 'report.docx', 'background', new AbortController().signal))
...
it.each(['workspaceFiles', 'fs'] as const)('reports unavailable file access when %s is absent', async (missing) => {
  ...
  try {
    ...
    await expect(independent.officeToPdf.render(agent, 'report.docx', 'foreground', new AbortController().signal))
