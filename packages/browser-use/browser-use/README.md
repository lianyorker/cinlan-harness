...
Shipped profiles do not mount this service or its optional providers. Add the service and one provider explicitly to the selected composition. The local Cinlan browser capability, BrowserUI, and sidebar use their independent packages; this registration does not select or replace them.
