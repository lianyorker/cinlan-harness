...
export function adapter(config: Config = {}) {
  ...
  return new DeepSeekMessagesAdapter({ connection: () => resolveAdapterOptions(Object.assign({ protocol: 'messages' } as Config, config)), apiKey: () => Promise.resolve('test-key'), userId: () => 'test-user', attachments: () => undefined, imageAccess: () => undefined, files: () => files, prepareExtensions })
}
