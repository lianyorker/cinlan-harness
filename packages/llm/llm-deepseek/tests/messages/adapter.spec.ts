...
describe('direct Messages HTTP', () => {
  ...
  it('uses the Messages endpoint, authentication, attribution and final usage', async () => {
    ...
    const response = await assemble(llm.stream(options({ model: 'deepseek-v4-flash-vision-exp', sessionId: SessionId('session-test'), purpose: 'compaction' })), 'deepseek-v4-flash-vision-exp')
    expect(response.message.content).toEqual([{ type: 'text', text: 'Hello 世界' }])
    expect(response.message.source).toMatchObject({
      model: 'deepseek-v4-flash-vision-exp', replayState: { response: { model: 'deepseek-v4-flash-vision-exp' } },
    })
...
describe('Cordis provider composition', () => {
  it('resolves an attachment service loaded after the adapter and maps its read-only path', async () => {
    ...
    await ctx.plugin(Messages, {
      protocol: 'messages', baseURL: http.url, models: [{ id: model, inputModalities: ['text', 'image'] }],
    })
    ...
    const attachment = await ctx.attachments.saveImage({ data: Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGP4z8AAAAMBAQDJ/pLvAAAAAElFTkSuQmCC', 'base64'), mediaType: 'image/png' })
  ...
  it.each([
    ...
    { model: 'deepseek-v4-pro', inHistory: false },
    { model: MODEL, inHistory: false },
    { model: MODEL, inHistory: true },
    { model: 'deepseek-v4-flash-vision-exp', inHistory: true },
    ...
    if (inHistory && model !== 'deepseek-flash') {
      await ctx.settings.update(Messages.name, { models: [{ id: model, systemPromptUpdate: 'in-history' }] })
    }
  ...
  it('continues a recorded tool turn with a warning when its native replay version is unknown', async () => {
    ...
    // Message payloads from the recorded degraded-replay session; unrelated log events stay outside this fixture.
    const fixture = await readFile(new URL('fixtures/degraded-replay.jsonl', import.meta.url), 'utf8')
