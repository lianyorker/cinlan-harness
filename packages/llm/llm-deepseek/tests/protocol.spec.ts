...
it('selects protocol-specific official roots only when no endpoint is configured', () => {
  ...
  for (const protocol of ['chat-completions', 'messages'] as const) {
    expect(resolveAdapterOptions({ protocol, baseURL: 'https://gateway.example/custom/v1', models })).toMatchObject({
      protocol, baseURL: 'https://gateway.example/custom/v1', models: [{ id: 'custom-model', name: 'My gateway' }],
    })
  }
})

it('rejects an unknown protocol before resolving request credentials', () => {
