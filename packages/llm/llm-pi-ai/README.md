...
A profile's `models` list replaces the route's installed catalog; each entry defaults its unset fields from the installed model of the same id. Use it to narrow a route, correct capacities, or declare a model absent from the installed catalog. `modelOverrides` customizes individual installed models while retaining the rest. New or changed providers reject overrides beside a `models` list, on a hand-declared route, or naming a model the catalog does not describe. Stored catalog failures remain available for repair as described in [Failures and recovery](#failures-and-recovery).
