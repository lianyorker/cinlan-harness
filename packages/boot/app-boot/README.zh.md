...
带 `patchReload: live` 的 profile 会监视两份用户 patch 文件和 manifest 中选中的 bundle 列表。launcher 提供不可变的 `ProfileContext` 信息；模块替换、配置监视器与管理写入共用 HMR 生命周期队列，每个世代在应用各层前刷新本地解析器的包链接。被拒绝的编辑让最后一个可用应用继续运行。`startup` profile 既不安装这些监视器，也不安装 launcher 的仅监视 HMR 回退。
