...
A profile is how one dsh installation ships different app surfaces: `web`, `headless`, `acp`, `sdk`, `sdk-minimal`, and `execution-host` start distinct compositions from the same launcher. The explicit [device-control profile](../../../docs/user/develop/practice/device-control.md) enables desktop and mobile Providers. A profile lives at `$DSH_HOME/profiles/<name>` and combines installable bundles, its own `cordis.patch.yml`, and `patchReload: live | startup`; omitted reload policy keeps the historical `live` default for custom profiles. The shipped `web`, `browser`, `device-control`, `security-research`, and `work-items` templates use live reload; the stdio and headless templates apply patches only at startup. `sdk-minimal` and the [execution-host worker](../../bundle/execution-host-app/README.md) each name a standalone bundle; the other templates retain base-plus-mode stacks. The worker exports no filesystem roots until configured on its target and starts no Agent or HTTP server. `dsh --profile <name> --from-default-profile <template>` creates a custom profile at a new non-shipped name from one shipped template, while `dsh plugin` initializes a base-backed profile and manages its installed bundles. A missing bundle or one without a patch declaration fails startup loudly. Application-owned npm projects, such as Electron's reserved Desktop profile, use `loadProfileDirectory` to load an already initialized directory without exposing it through CLI profile lookup.

The default Web template composes base, Web, Browser, Computer Use, then [Web capability defaults](../../bundle/web-capability-defaults/README.md). Browser and native Computer Use entries exist but remain disabled until the official Plugin Manager writes a user override. The `browser` and `device-control` templates retain their explicit enabled providers. Loading an installed Web profile upgrades only the exact legacy `[base, web-app]` bundle tuple; customized lists, explicit reload policy, and user patch bytes remain unchanged.

Before Loader evaluates profile configuration, the CLI provides `dshProfileName` from the loaded profile. Electron supplies its reserved `desktop` identity, including staged launches. Profile-scoped state owners can use `profile: !!js dshProfileName`; mutable state remains under the Harness home, separate from replaceable installed profile packages. Lower-level `boot()` embedders supply their own explicit profile identity when composing those owners.
...
Profiles with `patchReload: live` watch both user patch files and the manifest's selected bundle list. The launcher supplies immutable `ProfileContext` facts; module replacement, configuration watchers, and management writes share the HMR lifecycle queue, and each generation refreshes the local resolver's package links before applying its layers. A rejected edit leaves the last good app running. A `startup` profile installs neither those watchers nor the launcher's watch-only HMR fallback.
