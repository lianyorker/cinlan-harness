...
import {
  ...
  prepareProfilePatches, runProfileConfiguration,
...
export class PluginManager extends TypertRemoteService {
  ...
  async listPlugins(): Promise<PluginInfo[]> {
    const rows = flatten(composeEntries([this.ownerContext.get('pluginManagementHost')?.readPatches()
      ?? readProfilePatches('dsh', this.profile)]))
    ...
    return snapshot.entries.map((entry) => {
      ...
      if (protectedModules.has(entry.moduleName) || entry.entryId === this.ownerEntryId
        || this.ownerContext.get('pluginManagementHost')?.protectedIds.includes(actual?.options.id ?? '')) {
  ...
  listBundles(): Promise<BundleInfo[]> {
    ...
    for (const name of names) {
      ...
      try {
        ...
        const readOnlyReason = this.shellOwned() || this.protectsManager(name) ? 'management-required' as const : undefined
  ...
  async inspect(spec: string, signal?: AbortSignal): Promise<PluginSpecInspection> {
    if (this.shellOwned()) return refused('unknown', 'Use the Desktop Plugins window to install packages.')
    let parsed
    try {
  ...
  setPluginEnabled(id: PluginEntryId, enabled: boolean): Promise<ChangeResult> {
    return this.change(result => this.configure(async () => {
      ...
      return current?.enabled !== enabled && this.liveReload() ? 'overridden' : undefined
  ...
  removeBundle(name: string): Promise<ChangeResult> {
    return this.change(async (result) => {
      await this.configure(async () => {
        ...
        if (!this.liveReload() && (this.profile.startedBundles.includes(name)
  ...
  private configure<T>(operation: () => Promise<T>): Promise<T> {
    return runProfileConfiguration(this.ownerContext, () => { this.abort.signal.throwIfAborted(); return operation() })
  }

  private liveReload(): boolean {
    return this.profile.patchReload === 'live' && this.ownerContext.get('hmr') !== undefined
  }

  private async reload(requiredIds: readonly string[] = []): Promise<string[]> {
    if (!this.liveReload()) return []
    const patches = this.ownerContext.get('pluginManagementHost')?.readPatches()
      ?? await prepareProfilePatches('dsh', this.profile)
    return reconcileProfilePatches(this.ownerContext.root, patches, 'dsh', requiredIds)
  }

  private shellOwned(): boolean {
    return this.profile.name === 'desktop' || this.ownerContext.get('dshProfileName') === 'desktop'
      || this.ownerContext.get('pluginManagementHost') !== undefined
  }
  ...
  private async change(
    ...
    if (this.shellOwned() && (reason !== 'plugin' || this.ownerContext.get('pluginManagementHost') === undefined)) {
    ...
    return withFileLock(join(this.profile.dir, 'package.json'), async () => {
      ...
      const result: ChangeResult = { ...request, changed: false,
        application: this.liveReload() ? 'applied' : 'restart-required' }
      try {
