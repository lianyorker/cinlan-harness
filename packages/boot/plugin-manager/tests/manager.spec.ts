...
import {
  ...
  runProfileConfiguration, watchProfilePatches,
  type ProfileContext,
...
async function fixture(reload: 'live' | 'startup' = 'live', overlay = false, prepare?: (ctx: Context) => void, config: Config = {}, packageManager?: ProfileContext['packageManager'], profileName = 'test') {
  ...
  const profile: ProfileContext = {
    name: profileName, patchReload: reload,
...
it.each(['profile', 'launcher'])('refuses Desktop mutations at the executor using %s identity before touching profile files', async (identity) => {
  const { manager, dir } = await fixture('startup', false,
    identity === 'launcher' ? (ctx) => { ctx.provide('dshProfileName', 'desktop') } : undefined,
    {}, undefined, identity === 'profile' ? 'desktop' : 'test')
  ...
  for (const operation of [
    ...
    () => manager.setBundleEnabled('extra', false),
    () => manager.installBundle('external'),
    () => manager.removeBundle('extra'),
  ]) {
    expect(await operation()).toMatchObject({ changed: false, application: 'failed', error: { code: 'management-required' } })
  }
...
it('applies Desktop row changes through its launcher and keeps package transactions in the shell', async () => {
  const readPatches = vi.fn<(ctx: Context) => PatchOptions[]>(ctx => readProfilePatches('test', ctx.profileContext))
  const { ctx, manager, dir } = await fixture('live', false, (ctx) => {
    ctx.provide('pluginManagementHost', { protectedIds: ['required'], readPatches: () => readPatches(ctx) })
  }, {}, undefined, 'desktop')
  const run = vi.spyOn(operations, 'runProfilePnpm')
  const view = vi.spyOn(operations, 'viewProfilePackage')
  onTestFinished(() => { run.mockRestore(); view.mockRestore() })
  const manifest = readFileSync(join(dir, 'package.json'), 'utf8')
  const id = (await manager.listPlugins()).find(row => row.patchId === 'managed')!.entryId
  ...
  expect(readPatches).toHaveBeenCalled()
  expect((await manager.listBundles()).every(row => !row.removable && row.readOnlyReason === 'management-required')).toBe(true)
  for (const operation of [
    () => manager.setBundleEnabled('extra', false),
    () => manager.installBundle('external'),
    () => manager.removeBundle('extra'),
  ]) expect(await operation()).toMatchObject({ changed: false, application: 'failed', error: { code: 'management-required' } })
  expect(await manager.inspect('external')).toMatchObject({ status: 'refused' })
  expect(run).not.toHaveBeenCalled()
  expect(view).not.toHaveBeenCalled()
  expect(readFileSync(join(dir, 'package.json'), 'utf8')).toBe(manifest)
})

it('refuses a launcher-protected Desktop row before persisting a patch', async () => {
  const { manager, dir } = await fixture('live', false, (ctx) => {
    ctx.provide('pluginManagementHost', { protectedIds: ['managed'], readPatches: () => readProfilePatches('test', ctx.profileContext) })
  }, {}, undefined, 'desktop')
  const patch = readFileSync(join(dir, 'cordis.patch.yml'), 'utf8')
  const row = (await manager.listPlugins()).find(row => row.entryId === 'include:managed')!
...
it('describes a bundle by its manifest and patch: one-liner, rows without a live entry, and the built-in rows it changes', async () => {
  ...
  writeFileSync(join(dir, 'node_modules', 'described', 'cordis.patch.yml'), JSON.stringify([
    { insert: [{ id: 'described-row', name: './plugin.mjs' }, { name: './plugin.mjs', config: { service: 'anonymousProbe' } }] },
    { id: 'managed', disabled: true }, { id: 'described-row', config: { service: 'describedProbe' } },
  ]))
  const manifest = readProfileManifest('test', dir)
  ...
  expect(await manager.setBundleEnabled('described', true)).toMatchObject({ application: 'applied' })
  ...
  expect(await manager.setBundleEnabled('described', false)).toMatchObject({ application: 'applied' })
...
it('runs a real pnpm dependency script only after approval and retry', async () => {
  const pnpm = fileURLToPath(new URL('./bin/pnpm.mjs', import.meta.resolve('pnpm')))
  const { manager, dir, profile } = await fixture('startup', false, undefined, {}, {
...
it('reports a selected plain dependency as a problem, omits an unselected one, and reports missing versions', async () => {
  const { manager, dir, profile, stopHmr } = await fixture()
...
it.each(['activation', 'import'])('rolls back runtime entries after %s failure, retains the saved selection and retries corrected rows', async (failure) => {
  const { manager, dir, bundle, ctx } = await fixture()
  const healthy = { id: 'healthy', name: './plugin.mjs', config: { service: 'healthyProbe' } }
  bundle('broken', [healthy, failure === 'activation'
    ? { id: 'broken', name: './plugin.mjs', config: { fail: true } }
    : { id: 'broken', name: './missing.mjs' }])
  const before = await manager.listPlugins()
  const result = await manager.setBundleEnabled('broken', true)
  expect(result).toMatchObject({ changed: true, application: 'failed', error: { code: 'operation-error' } })
  expect(result.error?.diagnostic).toContain(failure === 'activation' ? 'test activation failed' : 'missing.mjs')
  expect(readProfileManifest('test', dir).dsh?.profile?.bundles).toEqual(['core', 'extra', 'broken'])
  expect(await manager.listPlugins()).toEqual(before)
  expect(ctx.get('healthyProbe')).toBeUndefined()
  expect(ctx.get('managedProbe')).toBe(true)
  expect((await manager.listBundles()).find(row => row.name === 'broken')).toMatchObject({ enabled: true })
  expect((await manager.listBundles()).find(row => row.name === 'broken')?.rows.every(row => row.entryId === undefined)).toBe(true)
  expect(await manager.setBundleEnabled('broken', true)).toMatchObject({ changed: false, application: 'failed' })
  bundle('broken', [healthy, { id: 'broken', name: './plugin.mjs', config: { service: 'recoveredProbe' } }])
  expect(await manager.setBundleEnabled('broken', true)).toMatchObject({ changed: false, application: 'applied' })
  expect(ctx.get('healthyProbe')).toBe(true)
  expect(ctx.get('recoveredProbe')).toBe(true)
  expect((await manager.listPlugins()).find(row => row.patchId === 'broken')).toMatchObject({ enabled: true, fiberPhase: 'active' })
})

it('returns unchanged pending entries as warnings while toggling and removing another bundle', async () => {
  const { manager, dir, bundle, ctx } = await fixture()
  bundle('broken', [
    { id: 'broken', name: './pending.mjs' },
    { id: 'missing', name: './pending.mjs' },
  ...
  expect(changed.warnings?.every(warning => warning.includes('pending (waiting for service: unavailable)'))).toBe(true)
  expect(ctx.get('managedProbe')).toBeUndefined()
...
it('offers installation-owned bundles switched off and never removable', async () => {
  const { manager, profile } = await fixture()
  const offered = 'installation-bundle'
  ...
  expect((await manager.listBundles()).find(row => row.name === offered)).toMatchObject({
    enabled: true, optional: false, removable: false,
  })
