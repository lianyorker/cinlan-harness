...
Use the shipped `spawn` and `fork` providers for delegation. They execute Harness Agents and preserve the same Session, model, tool, and permission mechanisms as the parent. Keep their control and job tools available so users can inspect, steer, cancel, and collect delegated work. Product presets do not install external CLI Agents or expose their account and runtime configuration.
