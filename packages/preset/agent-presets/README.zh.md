...
从 preset 组装的会话会运行该 preset `agent.cordis.yml` 所列插件：它的工具、提示词段落与 skill。加入同一 preset 的本地会话共享一份已安装的组装，且各会话的状态彼此隔离。远程会话在已准入的执行提供方上下文中挂载 preset，挂载随 Agent 生命周期释放。子 agent（subagent）会加入其父方的组装，因此它看到的工具与提示词段落和创建它的 agent 相同。
...
本地会话只有在尚未产出任何内容——没有消息或工具调用——时才能切换到不同的 preset。此后组装在会话的生命周期内固定，因为在对话中途调换工具会留下新组装无法执行的已记录工具调用。已提交的切换会发出 `tools/change`，因为解析后的工具集在没有注册表编辑的情况下发生了变化。切换也会记入会话日志，因此恢复或 fork 的会话会按它运行的组装重建。
...
- **本地常驻组装。** 本地 preset 在进程内只挂载一次，挂到常驻 scope 之下；agent 通过把自己的 scope key 认父到该挂载来加入，因此挂载的注册与监听器覆盖每个已加入的 agent，而不覆盖兄弟 preset 的。
...
远程准入在执行绑定设置完成后调用 `mountInExecution`。它在已准入的提供方上下文下持有独立作用域，按执行平台选择随附 shell 行，并在 Agent 回滚或释放时释放作用域。子级组合保留父级挂载。远程 preset 变更会被拒绝，包括首条消息之前。随附远程 preset 不启用指令文件和文件系统 skill 发现，直到这些消费方支持执行位置对应的路径规则与监听器。
...
- 远程 Session 不能更改 preset。随附组合不启用指令文件和文件系统 skill 发现，因为这些消费方使用 Host 路径规则与监听器。
