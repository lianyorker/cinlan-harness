...
const originalCmdlineInternals = { ...cmdlineInternals }
...
afterEach(async () => {
  try {
    ...
    Object.assign(cmdlineInternals, originalCmdlineInternals)
    Object.assign(startupInternals, originalInternals)
...
async function bootStartup(
  ...
  writeFileSync(join(dir, 'cordis.yml'), [
    ...
    '- id: headless-startup',
    '  name: cordis:headless-startup',
    '',
  ].join('\n'))
  const observing = { write: (chunk: string) => { observed.out += chunk; return true } }
  // Track stderr separately so JSON-mode assertions detect stray diagnostics.
  const observingErr = {
    write: (chunk: string) => {
      observed.out += chunk
      observed.err += chunk
  ...
  ctx.loader.builtins['headless-startup'] = startup
  ctx.loader.builtins['headless-observer'] = {
    apply: (_ctx: Context, config: unknown) => { observed.runnerConfig = config },
  }
...
describe('headless command-line provider', () => {
  ...
  it('writes a JSON error when the session option has no value', async () => {
    const { task, observed } = await bootStartup(['--json', '--session-id'])
    expect(JSON.parse(observed.out.trim())).toEqual({
      type: 'error', message: "option '--session-id <id>' argument missing",
    })
    expect(task).toBeUndefined()
    expect(observed.err).toBe('')
    expect(observed.exits).toEqual([1])
