...
- **Windows foreground activation is best effort** — synthesized input can be suppressed on secure desktops, restricted remote sessions, or when an elevated window is focused, so the dialog may still open behind other windows. The Alt press may briefly highlight the focused window's menu bar.
