...
describe('web e2e: archived sessions are restored from the Settings page', () => {
  ...
  it('archives the seed, unarchives it from Settings, and keeps it restored across reload', async () => {
    ...
    const search = settings.getByRole('searchbox', { name: 'Search archived sessions' })
    await search.waitFor({ timeout: 10_000 })
    const unarchiveRow = settings.getByRole('button', { name: `Unarchive ${title}` })
    await expect.poll(() => unarchiveRow.count(), { timeout: 10_000 }).toBe(1)
    expect(await settings.getByText(title, { exact: true }).count()).toBe(1)
    expect(await settings.getByRole('button', { name: `Open ${title}` }).count()).toBe(0)
    expect(await settings.getByText(/^Ungrouped · /).count()).toBe(1)
    expect(await settings.getByText('No archived sessions.', { exact: true }).count()).toBe(0)
    // The search box filters these rows in both directions.
    await search.fill('zzz-no-such-archived-session')
    await expect.poll(
      () => settings.getByText('No matching sessions.', { exact: true }).count(),
      { timeout: 5_000 },
    ).toBe(1)
    expect(await unarchiveRow.count()).toBe(0)
    await search.fill(title)
    await expect.poll(() => unarchiveRow.count(), { timeout: 5_000 }).toBe(1)
    expect(await settings.getByText('No matching sessions.', { exact: true }).count()).toBe(0)
    await search.fill('')
    await expect.poll(() => unarchiveRow.count(), { timeout: 5_000 }).toBe(1)

    const settingsSearch = settings.getByRole('searchbox', { name: 'Search settings...' })
    await settingsSearch.fill('Restore sessions')
    await settings.locator('button').filter({ hasText: 'Search archived sessions' }).click()
    await expect.poll(() => search.evaluate(input => document.activeElement === input)).toBe(true)
    ...
    expect(await settings.getByText(title, { exact: true }).count()).toBe(0)
    await expect.poll(
      () => settings.getByText('No archived sessions.', { exact: true }).count(),
      { timeout: 10_000 },
    ).toBe(1)
    await settings.getByRole('button', { name: 'Back to app' }).last().click()
