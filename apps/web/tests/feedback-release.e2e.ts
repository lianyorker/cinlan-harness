...
describe.each(MODE === 'record' ? ['deepseek-official'] : ['deepseek-official', 'feedback-mock'])('web e2e: feedback release for %s', (provider) => {
  ...
  it('drives the recorded prompt to a settled turn (all modes)', async () => {
    ...
    if (MODE !== 'record') await selectModel(official ? 'DeepSeek-V4-Flash' : 'Feedback mock')
    ...
    if (MODE !== 'record') {
      expect(agent?.session.requestHeader()?.config.model).toBe(official ? 'deepseek-v4-flash' : 'feedback-mock')
      expect(agent?.session.requestContext()?.contextWindow).toBe(128_000)
      await page.getByRole('button', { name: '6% of context used', exact: true }).waitFor({ timeout: 10_000 })
    }
