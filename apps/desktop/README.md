...
Electron chooses typed English or Chinese shell copy from its application locale and falls back to English. The startup page, menus, native dialogs, and plugin-management renderer use the same locale payload; the repository Client UI i18n gate checks these desktop sources. Native Edit actions remain available on every platform; macOS also retains standard File, Window, and application visibility actions.

### Visible startup

The primary window displays `dsh-app://shell/startup.html` before profile reconciliation begins. This local page inherits the product's theme and typography and reports actual recovery, verification, installation, health-check, and backend startup stages without estimating a percentage. A worker performs profile preparation so synchronous archive and package work does not block Electron's window controls. The same window navigates to the matching application only after the backend is ready.

Closing during preparation requests cancellation and displays a stopping state. The shell waits for an active package operation to finish, lets transaction cleanup and any probe Host settle, and joins the worker before quitting; it does not terminate a filesystem transaction midway. A health-check Host that fails to stop keeps its transaction locked and its staging files intact while the page reports cleanup retries. Failures stay visible with a diagnostic path. Restart is offered only after safe cleanup; Exit can retry failed backend cleanup. The startup preload exposes only status, locale, and these lifecycle actions to the exact startup page; the application renderer still receives only its protocol marker.
