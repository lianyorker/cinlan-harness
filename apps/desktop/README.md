...
5. Boot and stop the complete staged backend as a health check. Startup has no active backend; a plugin mutation stops an already running backend before this check and restores it afterward. This serialization prevents two desktop backends from sharing `$DSH_HOME`; installation or plugin incompatibility before activation leaves the active profile unchanged and retains unused staging for later maintenance.
