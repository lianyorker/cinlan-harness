...
The main dsh renderer receives the desktop protocol marker and an origin-restricted action that opens the native Plugins window. The separate plugin window receives structured list, install, remove, update, and update-check operations; neither renderer receives filesystem access, raw Electron IPC, a shell, or arbitrary pnpm arguments.

Electron chooses typed English or Chinese shell copy from its application locale and falls back to English. The startup failure panel, menus, native dialogs, and plugin-management renderer use the same locale payload; the repository Client UI i18n gate checks these desktop sources. Native Edit actions remain available on every platform; macOS also retains standard File, Window, and application visibility actions.

### Plugin configuration and native applications

Settings exposes installed plugin controls and each plugin's configuration forms. Add opens the native Plugins window, which owns installation, updates, and removal. Individual plugin switches save the Desktop profile patch and apply to the running Host; after a successful switch, the renderer refreshes once to load its updated client plugin list while the Host keeps running. Mandatory Desktop transport and management rows cannot be disabled through these controls.
...
The Desktop Host mounts the [bundled Office skills](../../packages/skill/skill-office/README.md) and `load_workspace_dependencies` before reporting readiness, including during staged profile health checks. The three skills use ordinary files under `runtime/office-skills` outside ASAR; missing resources fail Host startup. Their availability does not require a rendering service.

The first runtime query installs the bundled `runtime/primary-runtime` payload into `$DSH_HOME/dsh-runtimes/dsh-primary-runtime` and returns absolute Python, Node.js, pnpm, and package paths. Installation copies local payload bytes without downloading packages or changing PATH. Matching metadata reuses the installed release; changed payload identity replaces its dependency tree. The authoring payload uses its own [locked interpreter and wheel versions](scripts/primary-runtime-lock.json), independently of the Node.js executable that runs the Host.

### Visible startup

The primary window displays the shared `HARNESS` / `Loading plugins…` spinner at `dsh-app://shell/startup.html` before profile reconciliation begins. Normal loading shows only the application's [boot view](../../packages/client/web/README.md#use-this-package); preparation stages remain internal. A worker performs profile preparation so synchronous archive and package work does not block Electron's window controls. After the backend is ready, the same window navigates to `dsh-app://app/index.html`, where the application creates the same view. This handoff replaces the document; the DOM and spinner animation do not persist across navigation.

Native window close during preparation requests cancellation. The shell waits for an active package operation to finish, lets transaction work and any probe Host settle, and joins the worker before quitting; it does not terminate a filesystem transaction midway. A health-check Host that fails to stop keeps its transaction locked and its staging files intact while cleanup retries. Only startup errors reveal the localized failure panel with error details and an available diagnostic path. Restart is offered only after safe backend cleanup; Exit can retry failed backend cleanup. The startup preload exposes only status, locale, and these lifecycle actions to the exact startup page; the application renderer receives its protocol marker, native Plugins window action, and semantic update status/open operations.
...
|---|---|
| `integrity.json` and `desktop-packages.json` | Verify every inventoried seed file, local tarball hash, and the bound dsh and Desktop Host versions before installing or reconciling a profile. |
...
1. Under the transaction lock, recover an interrupted activation journal, validate the seed release metadata and core package set, and require the seed version to equal Electron's application version.
2. Reuse the active profile only when its parsed release metadata, installed dsh and Desktop Host versions, and freshly verified core package set match the seed, including every tarball's name, version, filename, size, and SHA-512 integrity. Reuse skips the full seed integrity inventory and store archives; same-version core content changes still require reconciliation, and corrupt core tarballs fail verification.
3. Otherwise verify the complete seed integrity inventory before validating every archive entry, extracting all store shards into a temporary Desktop-owned staging directory, merging the package files and SQLite package-index records into the private store, creating a staging profile, and running `pnpm install --offline --frozen-lockfile --trust-lockfile` through the bundled Node.js and pnpm. Seed records replace matching index keys while plugin-only records remain available.
...
5. Boot and stop the complete staged backend as a health check. Startup has no active backend; a plugin mutation stops an already running backend before copying metadata and keeps it stopped until activation or safe failure recovery. This serialization prevents two desktop backends from sharing `$DSH_HOME`; installation or plugin incompatibility before activation leaves the active profile unchanged and retains unused staging for later maintenance.
6. Atomically persist each next activation phase before its directory move, move the active profile to `$DSH_HOME/desktop/rollback/profile`, and move staging into `$DSH_HOME/profiles/desktop`. Windows directory moves and cleanup use bounded retries for transient file-handle errors. Recovery restores rollback for interrupted activation, retains the journal when restoration is incomplete, and retains an active profile whose journal reached `committed`. Retired rollback and failed replacement profiles move to fresh UUID staging roots for later deletion; a partially started replacement backend must stop before restoration.

After the main application loads and after plugin mutations, the shell runs `cleanupOrphanedStaging(signal?)` asynchronously under the transaction lock. A pending journal or a retained non-active Host prevents cleanup. Maintenance removes only direct UUID staging roots, unlinking root symlinks or junctions without following them. Invalid or unowned staging entries and removal failures produce maintenance warnings and leave residue for a later run. Plugin mutations await current maintenance. Exit cancels between roots and waits for any removal already in progress; orphan cleanup after canceled startup waits for a later successful startup.
...
`dev:desktop` builds the current Host, client bundles, Web frontend, and Electron shell, projects the built CLI and private Desktop Host packages with their workspace dependencies into a disposable desktop npm project, prepares the locked Office payload and external skill assets, and launches Electron under the invoking Node.js without resolving dsh from npm:
...
With existing build artifacts and an interactive desktop session, run `node apps/desktop/tests/settings.integration.mjs` from the repository root to verify Settings in Electron. Development mode uses `start:desktop` with pnpm automatic dependency installation disabled. To check an existing Windows unpacked application, add `--packaged '<absolute path to DeepSeek Harness.exe>'`; that mode verifies the packaged ASAR and bundled runtime, saves Git and terminal preferences through the UI, and checks persistence after a full process restart. Both modes isolate Harness home and Electron userData and check Chinese/English layout, search, focus, close actions, and language persistence. Capability navigation retains Security Research, Browser, Computer Use, and Mobile; Design is absent from navigation and search. Packaged first launches also require the shared loading spinner before application navigation and record internal startup stages, main-process response latency, and elapsed times; observation times include debugger attachment and test overhead. Screenshots, accessibility trees, artifact hashes, and the result record remain under `.artifacts/desktop-settings-*`; the check shuts down its application, verifies debugger port release, and removes its temporary root. It does not build artifacts, run an installer, or execute terminal commands.

For startup lifecycle checks, combine `--packaged` with `--close-during-startup` to close the native window during actual installation and verify transaction-lock release and preservation of the active profile, then reopen the same home with retained staging and require the application's onboarding dialog. Orphan staging cleanup waits until the application loads. Combine `--packaged` with `--fail-profile` to verify the persistent error page and diagnostic file using a malformed release record created only in the temporary home. These modes do not continue into Settings. An optional `--existing-profile '<detached default-only profile>'` exercises same-version replacement and identical restart, or preservation of the old profile during cancellation; it never modifies the source snapshot and cannot be combined with `--fail-profile`.

After `pnpm run build:desktop`, the owner-local diagnostic `node apps/desktop/tests/startup-reuse.perf.mjs` times only `applyRelease()` returning `false` for an identical profile. It uses 320 synthetic 64 KiB core tarballs and 16 synthetic 34 MiB archives with a warm OS file cache; fixture construction, model calls, network, backend startup, and UI are outside the measured interval.
...
Electron owns one updater and mandatory-policy client. Metadata checks use completion-based retry deadlines and never download automatically. A user requests a download, then confirms installation against its exact verified version. Installation inspects running or queued agents and jobs, locks the native Host's API admission, drains admitted requests, rechecks tasks, and requires shutdown acknowledgement plus a clean child exit before handing off to the installer. A failed installer restores one native Host only when the previous process is confirmed stopped; unresolved exit blocks another installation and plugin mutation. The next launch reconciles the version-bound seed before reopening the product window.

Mandatory policy metadata contains the native application ID, selected HTTPS origin, allowed page origins, and anonymous production or isolated test-login authentication. An unconfigured local test package leaves policy disabled; production requires its origin. Known mandatory decisions survive transient check failures until a valid decision clears them. Mandatory UI closes floating and plugin windows and blocks new opens, but does not stop existing tasks without installation consent. Product preloads expose semantic status and open actions, while shell-only dialogs own consent.

The optional absolute DSH_DESKTOP_UPDATE_JOURNAL_DIR stores exclusive, flushed JSONL evidence with fixed action/error categories and no raw diagnostics, URLs, credentials, or transcript content. It does not replace the pending activation journal or restore policy state. Fatal recovery stops native Hosts before backing up the profile patch and resetting enabled bundles under the existing Desktop transaction lock; installed packages, Settings, and Harness-home data remain intact.
...
`DSH_DESKTOP_NODE_BINARY`, `DSH_DESKTOP_PNPM_ENTRY`, `DSH_DESKTOP_SEED_DIR`, `DSH_DESKTOP_PRIMARY_RUNTIME`, and `DSH_DESKTOP_DEV_PROJECT_DIR` select explicit resources for an unpackaged Electron process. Packaged applications ignore these variables and resolve signed resources from `process.resourcesPath`.

## Known limitations

- Desktop carries "Open In..." through Connection Fetch over `dsh-app://` and framed pipes without a `webServer`. The action requires a Session working directory and a resolved local application; inherited SSH launches hide it. The shipped composition leaves the separate Session/Settings `nativeOpen` override unset, so default-application opening uses platform detection (Windows/macOS, or Linux with WSL or a display server). Actual external application launches remain unverified in the current migration acceptance.
