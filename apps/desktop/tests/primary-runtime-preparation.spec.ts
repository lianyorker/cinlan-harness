...
it.each(['headers', 'body'])('bounds stalled archive %s without caching partial bytes', async (phase) => {
  const root = await mkdtemp(join(tmpdir(), 'desktop-download-timeout-'))
  const abort = new AbortController()
  const failure = new DOMException('Archive deadline exceeded', 'TimeoutError')
  const timeout = vi.spyOn(AbortSignal, 'timeout').mockReturnValue(abort.signal)
  vi.stubGlobal('fetch', vi.fn((_url: string, options: RequestInit) => {
    expect(options.signal).toBe(abort.signal)
    const stalled = new Promise<never>((_resolve, reject) => {
      abort.signal.addEventListener('abort', () => { reject(failure) }, { once: true })
    })
    queueMicrotask(() => { abort.abort(failure) })
    return phase === 'headers' ? stalled : Promise.resolve({ ok: true, arrayBuffer: () => stalled })
  }))
  try {
    await expect(downloadPrimaryRuntimeAsset('https://archive.invalid/python.tar.gz', 'a'.repeat(64), root)).rejects.toBe(failure)
    expect(timeout).toHaveBeenCalledWith(600_000)
    await expect(readFile(join(root, 'a'.repeat(64)))).rejects.toMatchObject({ code: 'ENOENT' })
  } finally {
    vi.unstubAllGlobals()
    timeout.mockRestore()
    await rm(root, { recursive: true, force: true })
