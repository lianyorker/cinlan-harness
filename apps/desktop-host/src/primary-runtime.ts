 ...
 * @returns Validated component versions and target identifiers; rejects absent payload metadata with its required path.
 */
export async function readPrimaryRuntime(root: string): Promise<PrimaryRuntimeManifest> {
  ...
  let metadata: string
  try { metadata = await readFile(metadataPath, 'utf8') } catch (cause) {
    if ((cause as NodeJS.ErrnoException).code !== 'ENOENT') throw cause
    throw new Error(`primary runtime: bundled payload metadata is missing at ${metadataPath}; prepare the Desktop primary runtime before using workspace dependencies.`, { cause })
  }
