...
it('reports an absent bundled payload without creating an installation', async () => {
  const { source, root } = await fixture()
  await rm(source, { recursive: true })
  await expect(installPrimaryRuntime(source, root)).rejects.toThrow('bundled payload metadata is missing')
...
it.skipIf(process.platform === 'linux')('loads the real tool through Cordis, exposes installed paths, and unregisters on disposal', async () => {
  ...
  try {
    ...
    const unavailable = await ctx.tools.execute({ ...request, callId: ToolCallId('missing-payload') })
    expect(unavailable.isError).toBe(true)
    expect(unavailable.content).toEqual([expect.objectContaining({ type: 'text', text: expect.stringContaining('bundled payload metadata is missing') })])
    await writeFile(join(source, 'runtime.json'), JSON.stringify(manifest))
