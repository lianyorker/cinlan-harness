...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        for decision in [
            ...
            {"type": "subagent/catalog", "seq": 3, "time": 0, "data": {"version": 0, "childId": "child", "childCreatedAt": 1234, "mode": "continuable", "label": "image reviewer"}},
        ]:
            print(json.dumps({"jsonrpc": "2.0", "method": "session.event", "params": {"sessionId": params["sessionId"], "event": decision}}), flush=True)
        print(json.dumps({
            "jsonrpc": "2.0",
...
""".strip()
    ...
    decisions = [event for event in result.events if event["type"] in ("image/offload", "subagent/catalog")]
    assert decisions == [
        {"type": "image/offload", "seq": 2, "time": 0, "data": {"targets": [{"seq": 0, "imageIndexes": [0, 2]}]}},
        {"type": "subagent/catalog", "seq": 3, "time": 0, "data": {"version": 0, "childId": "child", "childCreatedAt": 1234, "mode": "continuable", "label": "image reviewer"}},
    ]
