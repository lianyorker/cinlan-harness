...
The Cordis preset exposes persistent profile management through the optional `plugin_manager` tool on the existing manager service. Each call, including inventory reads, requires Full access or one-call approval because installed Host code runs outside workspace confinement. The tool reports saved-state and activation outcomes separately; startup profiles require restart. The same manager continues to enforce Desktop ownership and protected entries. A real Loader fixture verifies that a tool request persists a patch without pretending a startup-only profile has activated it.
