...
组合范围为 headless 或自定义配置组合中的 Linux/macOS 客户端与辅助程序。Windows Desktop 不提供此 POSIX 运行时；假定可访问主机文件系统的 Web 和 Desktop 工作区消费方需要独立集成。网络限制、进程可见性隔离、恶意主机证明、持久远端句柄及自动产物配置不属于本提供方家族。
