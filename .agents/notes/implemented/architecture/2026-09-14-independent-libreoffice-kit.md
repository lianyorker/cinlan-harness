...
The upstream API declares platform engines as optional dependencies. Desktop installs the kit as an external npm dependency. Conversion requires neither downloads nor GitHub credentials.
...
Changing the kit requires qualifying a release in its own repository and updating the Harness dependency versions and lockfile. A missing required npm package fails installation; Harness does not compile an engine to recover. The [conversion tests](../../../../packages/document/office-to-pdf/tests/conversion.spec.ts) validate installed kit conversion; Desktop packaging retains the external dependency and its engine materials.
