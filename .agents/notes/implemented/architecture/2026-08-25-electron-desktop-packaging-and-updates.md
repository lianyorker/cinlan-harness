...
Startup requires the packaged release version to equal Electron's application version. Before reusing `.dsh/profiles/desktop`, it verifies the active local tarballs and compares the normalized release metadata, installed dsh and Desktop Host versions, and complete core-package descriptor against the seed. Equal version numbers alone do not establish content identity: every tarball's name, version, filename, size, and SHA-512 integrity must match, as must the bundled Node.js and pnpm versions. Any difference enters the existing staging transaction, which installs the seed manifest and lockfile with `pnpm install --offline --frozen-lockfile --trust-lockfile` and restores every registered plugin bundle at its exact installed version through one offline pnpm add from the existing desktop store and metadata cache. The complete graph must pass the same health check before activation, while shared settings and Session data remain outside the replaced profile.

Recovery runs only after the transaction lock is acquired. The activation journal atomically replaces each persisted preparation, active-profile movement, staging activation, and commit record. Recovery restores rollback for an interrupted activation and retains the journal when restoration is incomplete; a committed journal retains the active profile, and any remaining UUID transaction root is retried under the lock. Windows profile moves and owned staging cleanup retry transient file-handle errors within a bounded interval. Orphan staging cleanup removes only direct UUID transaction roots. Startup failures write a stack trace to `$DSH_HOME/desktop/startup-error.log` unless an explicit diagnostic path is configured.
...
- The primary window reports startup progress before profile preparation, and loads the application only after its active backend and Web UI report the same dsh version and a compatible shell API.
