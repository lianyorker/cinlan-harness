...
The [target packaging script](../../../../apps/desktop/scripts/package-target.ts) invokes installed validation, bundling, and electron-builder entry points through the current Node executable. Resolving those tools directly prevents a package-manager execution wrapper from synchronizing workspace dependencies during a build. Explicit pnpm build scripts, packing, and seed installation retain their existing ownership and validation. The repository postinstall loads the dev-only Lefthook package lazily and returns when production installation omits it.
...
- The local pipe carrier opens no loopback listener, and the sandboxed renderer cannot access arbitrary filesystem or Electron APIs. A paired HTTPS listener requires explicit Remote Access configuration.
