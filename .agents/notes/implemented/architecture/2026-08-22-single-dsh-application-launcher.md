...
Windows ACL workers also relaunch through the private bootstrap. An exact resolved runner entry in argv selects the worker and removes the packaging entry argument before import; otherwise normal CLI and subprocess dispatch apply. Isolated Node fixtures verify these routes without building a release executable.
