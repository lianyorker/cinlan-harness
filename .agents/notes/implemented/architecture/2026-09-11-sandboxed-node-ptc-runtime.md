...
Every provider lookup used by run(spec) comes from the Context captured when NodePtcRuntime is constructed. Cordis traceable calls can supply a caller-bound receiver whose this.ctx resolves Host services; that receiver is not an execution-world selector. The context-capture regression uses a shadowed caller proxy and requires the remote policy and providers.
