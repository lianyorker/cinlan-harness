...
run(spec) 使用的每个提供方查找都来自构造 NodePtcRuntime 时捕获的 Context。Cordis traceable 调用可能提供一个调用方绑定的 receiver，使 this.ctx 解析到 Host 服务；该 receiver 不是执行世界选择器。Context 捕获回归测试使用带 shadow 的调用方 proxy，并要求远端策略与提供方。
