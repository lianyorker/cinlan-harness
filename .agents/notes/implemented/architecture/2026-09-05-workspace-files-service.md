...
Two constraints frame any answer. Each request resolves its filesystem and sandbox policy from a Session execution lease; reads remain unconfined by the sandbox write policy, so a web-facing read endpoint must own every fence itself, and the fences must survive a symlink that leaves the workspace, which a string-prefix test cannot see. And `dsh-fs` exposed one raw-byte read, `readBytes(target, signal, maxBytes)`, which refuses any file longer than its cap: correct for an image the model ingests whole, useless for one window of a large file.

## Decision

File requests capture one Session execution lease and use its filesystem and sandbox policy for the whole bounded read or change-feed generation. Connection loss never selects Host files. The observation feed filters the emitting tool’s Agent Session before canonical containment, so equal paths in different execution environments cannot cross generations. Cancellation closes the iterator and releases its retained providers.
...
|---|---|---|---|
| Host | `api/workspace-files/tsconfig.host.json` | `src/index.ts` (`WorkspaceFiles`, `Config`, gates, pager), `src/changes.ts` (`WorkspaceChangeFeed`), `src/types.ts` (wire types, error codes) | `dsh-execution-binding`, `dsh-fs`, `dsh-sandbox-policy`, `dsh-typert-protocol`, `dsh-agent`, `dsh-session` |
