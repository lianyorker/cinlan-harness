...
The authenticated carrier supplies explicit caller authority and cancellation. Gateway binds delegated invocation, stream, and event policies to that authority as defined by [same-Host phone pairing](2026-09-20-same-host-phone-pairing.md); it retains descriptor, lookup, and business dispatch ownership. Local browser requests retain Connection authentication and the Host/Origin fence. Idempotency remains the business owner's responsibility.
