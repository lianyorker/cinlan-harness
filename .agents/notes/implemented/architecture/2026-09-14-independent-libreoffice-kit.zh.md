...
上游 API 将平台引擎声明为可选依赖。Desktop 将 kit 作为外部 npm 依赖安装。转换无需下载或 GitHub 凭据。
...
更新 kit 需要在其独立仓库验证并发布新版本，再更新 Harness 依赖版本和锁文件。必需的 npm 包缺失时安装失败；Harness 不通过编译引擎恢复。[转换测试](../../../../packages/document/office-to-pdf/tests/conversion.spec.ts)验证已安装 kit 的转换；Desktop 打包保留外部依赖及其引擎材料。
