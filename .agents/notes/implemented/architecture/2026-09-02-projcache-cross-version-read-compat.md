...
- SQLite accepts explicitly compatible older unit versions but has no `backupRecord`; schema-invalid records still reject the whole domain rather than following JSON per-record backup-and-skip. Shipped compositions route this domain to JSON.
