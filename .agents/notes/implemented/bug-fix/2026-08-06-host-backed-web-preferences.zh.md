...
Settings 命名空间注册与释放具有独立的提交后失效通知，因为它们会改变可用 schema 和解析值，而不会改变用户文档。因此，在首次 describe 读取之后启用的功能会通过已有绑定变为可用。读取期间被失效通知作废的 describe 应答不会发布，而会重新读取；通知处理器不能通过旧应答重新发布已经移除的注册项。生命周期测试覆盖 Loader 启用、停用与重挂载、观察者异常隔离以及已有 Client scope 绑定。
