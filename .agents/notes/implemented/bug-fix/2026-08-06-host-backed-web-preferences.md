...
Settings namespace registration and disposal have their own committed invalidation event because they change available schemas and resolved values without changing the user document. A feature enabled after the initial describe read therefore becomes available through its existing bound scope. A describe response invalidated while in flight is discarded before publication and followed by a fresh read; notification handlers cannot resurrect a removed registration from a stale response. Lifecycle tests cover Loader enable/disable/remount, observer-failure containment, and already-bound Client scopes.
