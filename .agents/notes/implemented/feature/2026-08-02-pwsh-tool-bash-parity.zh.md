...
- **Windows 行为有明确测试**：输出前置代码在 FullLanguage 下将控制台和管道编码固定为 UTF-8，包括 Windows PowerShell 5.1。ConstrainedLanguage 保留控制台编码，读取 UTF-8 管道编码，不调用被禁止的构造函数或控制台 setter。真实受限 token 测试在 PowerShell 7 与 Windows PowerShell 5.1 的两种模式下验证多语言输出和空 stderr。提示词说明 Windows 强制终止以 exit 1 结算，不产生 signal marker。
