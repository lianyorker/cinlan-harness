...
- **Windows behavior has explicit tests**: the output preamble pins console and pipeline encoding to UTF-8 in FullLanguage, including Windows PowerShell 5.1. ConstrainedLanguage retains console encoding and reads the UTF-8 pipeline encoding without forbidden constructors or console setters. Real restricted-token tests exercise multilingual output and empty stderr in both modes with PowerShell 7 and Windows PowerShell 5.1. The prompts state that Windows forced termination settles as exit 1 without a signal marker.
