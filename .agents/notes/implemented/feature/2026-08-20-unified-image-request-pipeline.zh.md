...
`AttachmentStore.readImageRequest` 接收路由拥有的精确 `width`、`height` 和 `maxBytes` 目标。DeepSeek 默认使用公开的 token 网格、每边 4096 像素限制和 2MiB 编码字节目标；`imagePixelBudget` 显式选择总像素覆盖值（`low` 表示 512×512）。小图绝不放大。没有任何质量档位满足字节目标时，编码器保留质量阶梯中最小的产物（见[按 alpha 路由的质量阶梯记录](../bug-fix/2026-08-24-alpha-routed-image-quality-ladders.zh.md)）。普通轮次和辅助流使用相同派生过程。

`variantId` 和缓存路径覆盖规范化附件 ID、变换策略版本、路由尺寸和编码字节目标及固定编码参数。新缓存条目在发布前会完整解码。缓存命中只探测文件头，校验格式、8-bit sRGB/sRGBA、尺寸和透明通道，不会再次完整解码光栅；不匹配时会重新生成。因此，同一策略下的 DeepSeek Files 和 pi-ai 内联 base64 使用相同的确定性字节。内联计量使用派生字节经过 base64 膨胀后的长度，不使用规范化附件字节数。同一进程内相同 `variantId` 的调用共享一次变换和缓存写入。每个调用方可以取消自己的等待；只有全部等待方都取消时，共享变换才会中止。调用方对单数 `readImageRequest` 使用 `Promise.all` 保持结果顺序。本地实现通过一个 FIFO 限流器运行规范化和请求变换，`imageCompressionConcurrency` 的可配置范围为 1 至 8，默认值为 2。全部规范化附件准备完成后，批次仍按顺序发布。

请求大小引起的省略由[持久图片卸载事件](../architecture/2026-09-10-image-offload-events.zh.md)拥有。提供方按确切编码字节或 base64 字节计量并报告 `IMAGE_OFFLOAD_REQUIRED`；恢复插件在重试前记录选中的图片出现位置。已卸载图片渲染为占位文本，无需读取附件；追加式历史保留其原始引用。
