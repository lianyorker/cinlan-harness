...
每个公开入口都自行验证值域，而非依赖单一的面向模型配置路径。负值、小数、负零、非有限值、不安全整数、格式错误的存储父级深度以及推导溢出均被拒绝。直接的 `SubagentStartRequest` 可以省略上限，让此机制不约束深度。`dsh-tool-subagent` 在每次委派时通过 `SubagentRuntime.resolveMaxDepth()` 解析省略的工具覆盖；运行时读取共享设置，本地默认值为 `3`，显式部署值优先。数值工具覆盖优先于共享值；显式的 `'provider-managed'` 为由进程外提供方部署拥有递归预算的情形省略上限。三是一个较小的有限默认值，仍允许 root 加三代后代。[选择性集成决策](../architecture/2026-09-19-selective-upstream-integration-preserves-product-ownership.zh.md)在增加共享运行时设置的同时保留此默认值。提供方缺少 `depthLimit` 时，数值工具上限会在提供方挂载阶段失败。
