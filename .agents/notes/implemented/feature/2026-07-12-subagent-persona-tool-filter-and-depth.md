...
Every public entry validates the domain rather than relying on one model-facing configuration path. Negative values, fractions, negative zero, non-finite values, unsafe integers, malformed stored parent depth, and derived overflow all reject. A direct `SubagentStartRequest` may omit the cap to leave depth unbounded. `dsh-tool-subagent` resolves an omitted tool override through `SubagentRuntime.resolveMaxDepth()` for every delegation; the runtime reads shared settings with a local default of `3`, while explicit deployment values take precedence. A numeric tool override wins over the shared value; explicit `'provider-managed'` omits the cap for an out-of-process provider whose deployment owns its recursion budget. Three is a small finite default that still permits a root plus three descendant generations. The [selective integration decision](../architecture/2026-09-19-selective-upstream-integration-preserves-product-ownership.md) preserves that default while adding shared runtime settings. A numeric tool cap fails at provider mount when the provider lacks `depthLimit`.
