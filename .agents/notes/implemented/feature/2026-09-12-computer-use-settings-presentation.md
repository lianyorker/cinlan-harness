...
Mobile Emulator means Agent control of a device, not a mobile client connecting back to Harness. SDK detection, default-device persistence, and Cookie import have no Host API in this integration, so the pages provide explanations rather than simulated switches or installation buttons. Design Studio installation versus replacement remains undecided. The installed design Skill is guidance, not proof of a Design Studio Provider. GitHub publishing and downloadable dependency closure require a separate release implementation.
