...
**模型可见即已记录。** 运行时不变量断言模型请求可从日志重建。新增模型可见输入需要会话事件。改变已有消息的插件注册[纯消息投影](subsystems/session.zh.md#plugin-owned-message-projections)；脱离服务的读取方显式提供同一组定义。[图像卸载插件](subsystems/compaction.zh.md#image-offload)拥有其独立决策事件与解释逻辑。Surface 替换与消息投影推进内容代际，循环据此开启新的请求序列。
