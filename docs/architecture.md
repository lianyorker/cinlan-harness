...
**Model-visible means logged.** A runtime invariant asserts that model requests are reconstructable from the log. New model-visible input requires a session event. Plugins changing existing messages register [pure message projections](subsystems/session.md#plugin-owned-message-projections); detached readers supply the same definitions explicitly. The [image-offload plugin](subsystems/compaction.md#image-offload) owns its standalone decision event and interpretation. Surface replacements and message projections advance the content generation that the loop uses to start a new request series.
